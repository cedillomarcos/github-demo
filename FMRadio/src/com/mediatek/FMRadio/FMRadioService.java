...
public class FMRadioService extends Service implements FMRecorder.OnRecorderStateChangedListener {
    ...
    //GPBYY-248 shuchenlin begin
    private boolean mFMFocusGain = false;
    //GPBYY-248 shuchenlin end
    ...
    //zhuwei add
    private IFMRadioService.Stub mBinderaidl = new IFMRadioService.Stub(){
    	/*
        ...
        */
    	...
    	public void useEarphone(boolean isSpeaker){
  		...
  		public void onReceive(Context context, Intent intent) {
  			 if(intent.getAction().equals(Intent.ACTION_HEADSET_PLUG)){
  					if(0==intent.getIntExtra("state", 0)){//out
  						...
  						boolean result = tasks.get(0).baseActivity.getPackageName().contains("FMRadio");
  						if(!result){
							LogUtils.d(TAG, "Jiayusheng OnstopService()");
//  							Toast.makeText(FMRadioService.this,R.string.tost_noantenna_text, Toast.LENGTH_LONG).show();
  							Intent intent2 =new Intent();
  							intent2.setClass(FMRadioService.this, FMRadioService.class);
  							stopService(intent2);
  						}
				...
				//jiayusheng add GBLSSW-165 20130320 end
  			}
    ...
    public IBinder onBind(Intent intent) {
    	//zhuwei modify
         ...
         if(intent.getAction()!= null && intent.getAction().equals("com.mediatek.FMRadio.IFMRadioService"))
         { 
         	if(!isServiceInit())
    ...
    private class FMServiceBroadcastReceiver extends BroadcastReceiver {
		...
		if(mValueHeadSetPlug == 1){//headset plug out
		    mFmServiceHandler.removeMessages(FMRadioListener.MSGID_SCAN_FINISHED);
		    if(FMRadioActivity.isActivityDestroyed){
		        stopSelf();
		    }
		}
    ...
    private boolean powerUpFM(float frequency) {
        ...
        if (!requestAudioFocus()) {
            LogUtils.d(TAG, "FM can't get audio focus when power up");
//add cuinana GPBYY-188
        Toast.makeText(FMRadioService.this,R.string.toast_in_call,Toast.LENGTH_LONG).show();
//add cuinana GPBYY-188
            return false;
        }
        long time = System.currentTimeMillis();
    ...
    public boolean isRDSSupported() {
		...
		//modify by wangxiaobo GPBYL-154 20130515(start)
        //return isRDSSupported;
        return false;
		//modify by wangxiaobo GPBYL-154 20130515(end)
    }
...
//            enableFMAudio(true);
    ...
    class FmRadioServiceHandler extends Handler {
		...
		//GPBYY-123 yukai 20130902 begin--cancel delete
                bundle = new Bundle(2);
                bundle.putInt(FMRadioListener.CALLBACK_FLAG, FMRadioListener.MSGID_SWITCH_ANNTENNA);
                bundle.putBoolean(FMRadioListener.KEY_IS_SWITCH_ANNTENNA, isSwitch);
                notifyActivityStateChanged(bundle);
