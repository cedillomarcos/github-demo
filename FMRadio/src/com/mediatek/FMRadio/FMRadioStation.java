...
public class FMRadioStation {
    ...
    //MAOYUTAO GPBYL-105 20130515 start
 	public static boolean deleteAll(Context context) {
 		int rows = context.getContentResolver().delete(Station.CONTENT_URI, null, null);
 		//FMRadioLogUtils.d(TAG, "delete all rows = " + rows);
 		if (rows > 0) {
 			return true;
 		}
 		return false;
    ...
    //jiayusheng GBLLSW-251 20130311 start
 	public static boolean hasDuplicateName(Context context, String StationName){
 		Cursor cur = context.getContentResolver().query(Station.CONTENT_URI, 
 		new String[] {Station.COLUMN_STATION_NAME}, null, null,null);
 		
 			try{
 				if(cur!=null && cur.moveToFirst()){
 					while(!cur.isAfterLast()){
 						String name = cur.getString(0);
 						if(StationName == null || name.equals(StationName)){
							LogUtils.d(TAG, "hasDuplicateName--->name: " + name);
 							LogUtils.d(TAG, "hasDuplicateName--->StationName: " + StationName);
 							Toast.makeText(context, R.string.toast_channel_name_already_exist, Toast.LENGTH_LONG).show();
 							return true;
 						}
 						cur.moveToNext();
 					}
     ...
     //jiayusheng GBLLSW-251 20130311 end
    /**
