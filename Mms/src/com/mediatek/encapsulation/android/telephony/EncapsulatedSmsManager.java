...
public class EncapsulatedSmsManager {
    ...
    public static void setSmsMemoryStatus(boolean status) {
        if (EncapsulationConstant.USE_MTK_PLATFORM) {
            SmsManager.getDefault().setSmsMemoryStatus(status);
        } else {
    ...
    public static int copyTextMessageToIccCard(String scAddress, String address, List<String> text,
        ...
        if (EncapsulationConstant.USE_MTK_PLATFORM) {
            return SmsManager.getDefault().copyTextMessageToIccCard(scAddress, address, text,
                    status, timestamp);
        } else {
            return EncapsulatedGeminiSmsManager.copyTextMessageToIccCardGemini(
                  scAddress,address,text,status,timestamp,TelephonyManager.getDefault().getSmsDefaultSim());
        }
    ...
    public static ArrayList<String> divideMessage(String text, int encodingType) {
        if (EncapsulationConstant.USE_MTK_PLATFORM) {
            return SmsManager.getDefault().divideMessage(text, encodingType);
        } else {
    ...
    public static void sendMultipartTextMessageWithEncodingType(String destAddr, String scAddr,
        ...
        if (EncapsulationConstant.USE_MTK_PLATFORM) {
            SmsManager.getDefault().sendMultipartTextMessageWithEncodingType(destAddr, scAddr,
                    parts, encodingType, sentIntents, deliveryIntents);
        } else {
            /** M: Can not complete for this branch. */
            EncapsulatedGeminiSmsManager.sendMultipartTextMessageWithEncodingTypeGemini(destAddr, scAddr,
               parts, encodingType, TelephonyManager.getDefault().getSmsDefaultSim(), sentIntents, deliveryIntents);
        }
    }
