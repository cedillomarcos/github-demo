...
public class MmsConfig {
    ...
    public static void init(Context context) {
        ...
        /// M: add for iSMS
        initializeIsmsFilePath(context);
        loadSimInfo(context);
    }
    ...
    /// M: add for isms
    private static String mPicTempPath = "";
    private static String mAudioTempPath = "";
    private static String mVideoTempPath = "";
    private static String mVcardTempPath = "";
    private static String mCalendarTempPath = "";
    private static void initializeIsmsFilePath(Context context) {
        if (NmsCommonUtils.getSDCardStatus()) {
            mPicTempPath = NmsCommonUtils.getSDCardPath(context) + NmsCommonUtils.ISMS_FILE_PATH + "picture";
            File f_pic = new File(mPicTempPath);
            if (!f_pic.exists()) {
                f_pic.mkdirs();
            }

            mAudioTempPath = NmsCommonUtils.getSDCardPath(context) + NmsCommonUtils.ISMS_FILE_PATH + "audio";
            File f_aud = new File(mAudioTempPath);
            if (!f_aud.exists()) {
                f_aud.mkdirs();
            }

            mVideoTempPath = NmsCommonUtils.getSDCardPath(context) + NmsCommonUtils.ISMS_FILE_PATH + "video";
            File f_vid = new File(mVideoTempPath);
            if (!f_vid.exists()) {
                f_vid.mkdirs();
            }

            mVcardTempPath = NmsCommonUtils.getSDCardPath(context) + NmsCommonUtils.ISMS_FILE_PATH + "vcard";
            File f_vcard = new File(mVcardTempPath);
            if (!f_vcard.exists()) {
                f_vcard.mkdirs();
            }

            mCalendarTempPath = NmsCommonUtils.getSDCardPath(context) + NmsCommonUtils.ISMS_FILE_PATH + "calendar";
            File f_calendar = new File(mCalendarTempPath);
            if (!f_calendar.exists()) {
                f_calendar.mkdirs();
            }

            String cachePath = NmsCommonUtils.getCachePath(context);
    ...
    public static String getPicTempPath(Context context) {
        if (TextUtils.isEmpty(mPicTempPath)) {
            mPicTempPath = NmsCommonUtils.getSDCardPath(context) + NmsCommonUtils.ISMS_FILE_PATH + "picture";
            File f_pic = new File(mPicTempPath);
            if (!f_pic.exists()) {
                f_pic.mkdirs();
    ...
    public static String getAudioTempPath(Context context) {
        if (TextUtils.isEmpty(mAudioTempPath)) {
            mAudioTempPath = NmsCommonUtils.getSDCardPath(context) + NmsCommonUtils.ISMS_FILE_PATH + "audio";
            File f_aud = new File(mAudioTempPath);
            if (!f_aud.exists()) {
                f_aud.mkdirs();
        ...
        return mAudioTempPath;
    }

    public static String getVideoTempPath(Context context) {
        if (TextUtils.isEmpty(mVideoTempPath)) {
            mVideoTempPath = NmsCommonUtils.getSDCardPath(context) + NmsCommonUtils.ISMS_FILE_PATH + "video";
            File f_vid = new File(mVideoTempPath);
            if (!f_vid.exists()) {
                f_vid.mkdirs();
        ...
        return mVideoTempPath;
    }

    public static String getVcardTempPath(Context context) {
        if (TextUtils.isEmpty(mVcardTempPath)) {
            mVcardTempPath = NmsCommonUtils.getSDCardPath(context) + NmsCommonUtils.ISMS_FILE_PATH + "vcard";
            File f_vcard = new File(mVcardTempPath);
            if (!f_vcard.exists()) {
                f_vcard.mkdirs();
        ...
        return mVcardTempPath;
    }

    public static String getVcalendarTempPath(Context context) {
        if (TextUtils.isEmpty(mCalendarTempPath)) {
            mCalendarTempPath = NmsCommonUtils.getSDCardPath(context) + NmsCommonUtils.ISMS_FILE_PATH + "calendar";
            File f_calendar = new File(mCalendarTempPath);
            if (!f_calendar.exists()) {
                f_calendar.mkdirs();
        ...
        return mCalendarTempPath;
    }

    public static int getIpMessagServiceId(Context context) {
        return MessageUtils.getServiceManager(context).getIpMessageServiceId();
    }
    ...
    public static boolean isActivated(Context context) {
        if (!MessageUtils.getServiceManager(context).serviceIsReady()) {
            return false;
        }
        loadSimInfo(context);
        if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {
            ...
            boolean actStatus = MessageUtils.getServiceManager(context).isActivated();
    ...
    public static boolean isActivated(Context context, int simId) {
        ...
        boolean actStatus = MessageUtils.getServiceManager(context).isActivated(simId);
    ...
    public static boolean isServiceEnabled(Context context) {
        ...
        if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {
            ...
            boolean actStatus = MessageUtils.getServiceManager(context).isEnabled();
            MmsLog.d(TAG, "activation status:" + actStatus);
            return actStatus;
        }
    }
    ...
    public static boolean isServiceEnabled(Context context, int simId) {
        return MessageUtils.getServiceManager(context).isEnabled(simId);
    }
