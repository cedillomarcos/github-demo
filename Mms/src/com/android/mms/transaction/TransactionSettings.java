    ...
    public TransactionSettings(Context context, String apnName, int slotId) {
        ...
        if (EncapsulatedPhone.GEMINI_SIM_1 == slotId) {
            cursor = SqliteWrapper.query(context, context.getContentResolver(),
                                Uri.withAppendedPath(EncapsulatedTelephony.Carriers.SIM1Carriers.CONTENT_URI, "current"),
                                APN_PROJECTION, selection, null, null);
        } else if (EncapsulatedPhone.GEMINI_SIM_2 == slotId){
            cursor = SqliteWrapper.query(context, context.getContentResolver(),
                                Uri.withAppendedPath(EncapsulatedTelephony.Carriers.SIM2Carriers.CONTENT_URI, "current"),
                                APN_PROJECTION, selection, null, null);
        } else {
