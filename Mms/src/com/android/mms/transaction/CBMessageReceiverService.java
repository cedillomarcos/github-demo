...
import com.mediatek.encapsulation.android.telephony.EncapsulatedTelephony.CbSms.Intents;
...
public class CBMessageReceiverService extends Service {
    ...
    private static final Uri MESSAGE_URI = EncapsulatedTelephony.CbSms.CONTENT_URI;
    ...
    private final class ServiceHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (intent != null) {
                ...
                if (EncapsulatedTelephony.CbSms.Intents.CB_SMS_RECEIVED_ACTION.equals(action)
    ...
    private void handleCBMessageReceived(Intent intent) {
        ...
        EncapsulatedCbSmsMessage [] msgs = Intents.getMessagesFromIntent(intent);
        if (null == msgs) {
        ...
        if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
            EncapsulatedCbSmsMessage sms = msgs[0];
        ...
        if (messageUri != null) {
            if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {
                ...
                EncapsulatedCbSmsMessage msg = msgs[0];
                int slotId = msg.getMessageSimId();
                SIMInfo si = SIMInfo.getSIMInfoBySlot(getApplicationContext(), slotId);
                if (null != si) {
                    CBMessagingNotification.updateNewMessageIndicatorGemini(this, true, (int)si.getSimId());
                } else {
    ...
    private Uri insertMessage(Context context, EncapsulatedCbSmsMessage[] msgs) {
        return storeCBMessage(context, msgs);
    }

    // TODO Need replace with CB message
    private Uri storeCBMessage(Context context, EncapsulatedCbSmsMessage[] msgs) {
        EncapsulatedCbSmsMessage sms = msgs[0];
        ...
        MmsLog.d(MmsApp.TXN_TAG, "CB message body: " + body.toString());
        return resolver.insert(MESSAGE_URI, values);
    }
    
    // TODO  Need replace with CB Message
    private ContentValues getCBContentValue(EncapsulatedCbSmsMessage msg, String body) {
        ...
        if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {
            ...
            SIMInfo si = SIMInfo.getSIMInfoBySlot(getApplicationContext(), slotId);
            if (null != si) {
                values.put(EncapsulatedTelephony.CbSms.SIM_ID, (int)si.getSimId());
            } else {
                MmsLog.w(MmsApp.TXN_TAG, "getCBContentValue:SIMInfo is null for slot " + slotId);
            }
        }
        values.put(EncapsulatedTelephony.CbSms.DATE, Long.valueOf(System.currentTimeMillis()));
        // Channel ID is getting from getMessageID
        values.put(EncapsulatedTelephony.CbSms.CHANNEL_ID, msg.getMessageID());
        values.put(EncapsulatedTelephony.CbSms.READ, Integer.valueOf(0));
        values.put(EncapsulatedTelephony.CbSms.BODY, body);
        return values;  
    }
}
