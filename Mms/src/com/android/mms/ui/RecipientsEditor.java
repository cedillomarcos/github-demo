...
public class RecipientsEditor extends EncapsulatedMTKRecipientEditTextView {
 ...
 /// M:       setDropDownWidth(LayoutParams.MATCH_PARENT);
    ...
    public static CharSequence contactToToken(Contact c) {
        /// M: Just one log
        MmsLog.v(TAG, "RecipientsEditor:contactToToken, contact.getNameAndNumber=" + c.getNameAndNumber());
        /// M: change to use protosomatic number @{
        // SpannableString s = new SpannableString(c.getNameAndNumber());
        SpannableString s = new SpannableString(c.getmNameAndNumberProtosomatic());
        /// @}
        int len = s.length();
    ...
    private static String getNumberAt(Spanned sp, int start, int end, Context context) {
        ...
        if (!TextUtils.isEmpty(number)) {
            ...
            if ((pos >= 0 && pos < number.indexOf('>')) || number.endsWith(",") || number.endsWith(";")) {
    ...
    public ContactList constructContactsFromInputWithLimit(boolean blocking, int limit) {
        List<String> numbers = mTokenizer.getNumbers();
//modify GPBYL-132 wangxiaobo 20130516 (start)
        Collection<Long> contactIds = getContactIds();
        Collection<Long> dataIds = getDataIds();
        ArrayList<Long> contactIdsArray = new ArrayList<Long>();
        ArrayList<Long> dataIdsArray = new ArrayList<Long>();
        ...
        for (Long contactId : contactIds) {
            contactIdsArray.add(contactId);
        }
        for (Long dataId : dataIds) {
            dataIdsArray.add(dataId);
        }
        
        if (size == contactIdsArray.size() && size == dataIdsArray.size()) {
            Log.i(TAG, ">>>>>>>constructContactsFromInputWithLimit match size="+size);
            isMatch = true;
        }
//modify GPBYL-132 wangxiaobo 20130516 (end)
        ContactList list = new ContactList();
        ...
        for (String number : numbers) {
            if (count < limit) {
			  //modify GBLSSW-364 zhuyawen 20130328 (on)
                Contact contact = null;
                if (isMatch && count < size) {
                    contact = Contact.get(number, blocking, contactIdsArray.get(count), dataIdsArray.get(count));
                } else {
                    contact = Contact.get(number, blocking);
                }
                if (contact == null) {
                    contact = new Contact(number);
                }
             //modify GBLSSW-364 zhuyawen 20130328 (off)
                contact.setNumber(number);
                list.add(contact);
            } else {
    ...
    protected void updatePressedChip(boolean isChange) {
        updatePressedChip(mChipX, mChipY, isChange);
    }
    /// @}
}
