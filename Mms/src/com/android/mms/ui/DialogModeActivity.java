...
public class DialogModeActivity extends Activity implements
    ...
    private final static String TAG = "Mms/DialogMode";
    ...
    private final static int REQUEST_CODE_ECM_EXIT_DIALOG = 107;
    private final static String EXIT_ECM_RESULT = "exit_ecm_result";
    private final static String SELECT_TYPE = "Select_type";
    private final static int SIM_SELECT_FOR_SEND_MSG = 1;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        /// M: add for iSMS, notification listener
    ...
    private void initDialogView() {
        ...
        if (MmsConfig.isServiceEnabled(this)) {
            mReplyEditor.setHint(R.string.type_to_compose_text_enter_to_send_ipmsg);
        }
    ...
    public boolean isHome() {
        ...
        if (rtiSize <= 0) {
            ...
            if (homePackageNames.contains(packageName0)) {
                return true;
            } else {
                return false;
            }
...
//GPBYL-533 cuinana R.string.delete
    ...
    public void onClick(View v) {
        ...
        if (v.equals(mSmsContentText)) {
            ...
            if (isCurGroupIpMessage()) {
                /// M: open group chat
                openIsmsThread(getThreadId(), false);
            } else {
                openThread(getThreadId());
            }
            //deleteCurMsg();
    ...
    private void openThread(long threadId) {
        ...
        if (MmsConfig.getMmsDirMode()) {
			...
			MmsLog.d("liyang", "body:" + body);
            startActivity(ComposeMessageActivity.createIntent(this, threadId, body));
    ...
    private void showSimSelectedDialog(Intent intent) {
        ...
        for (int i = 0; i < mSimCount; i++) {
            ...
            if (MmsConfig.isServiceEnabled(this, (int) simInfo.getSimId())) {
                MmsLog.d(TAG, "show isms icon, simId = " + simInfo.getSimId());
                entry.put("isms", R.drawable.isms_sim_indicator_hissage);
            } else {
                MmsLog.d(TAG, "hide isms icon, simId = " + simInfo.getSimId());
                entry.put("isms", 0);
            }
            String simNumber = "";
    ...
    private void updateSendButtonState() {
        ...
        if (mSendButton != null) {
            ...
            if (enable) {
                if (isCurIpMessage()) {
                    mSendButton.setImageResource(R.drawable.ic_send_isms);
                } else {
    ...
    class TextLengthFilter implements InputFilter {
        ...
        public CharSequence filter(CharSequence source, int start, int end, Spanned dest,
            ...
            ArrayList<String> list = SmsManager.getDefault().divideMessage(text);
            if (list.size() > page) {
                maxLength = 0;
    ...
    private void showIpMessage() {
        ...
        if (isCurGroupIpMessage() && mCursor != null) {
            ...
            name = MessageUtils.getContactManager(this).getNameByNumber(name) + ":";
        ...
        IpMessage ipMessage = MessageUtils.getMessageManager(this).getIpMsgInfo(id);
    ...
    private void setIpTextItem(IpTextMessage textMessage) {
        ...
        int resId = com.android.mms.isms.util.message.SmileyParser.getInstance().getDynamicRes(textMessage.getBody());
        if (resId <= 0) {
            resId = com.android.mms.isms.util.message.SmileyParser.getInstance().getAdRes(textMessage.getBody());
        }
        if (resId <= 0) {
            resId = com.android.mms.isms.util.message.SmileyParser.getInstance().getXmRes(textMessage.getBody());
        }
        if (resId > 0) {
            ...
            resId = com.android.mms.isms.util.message.SmileyParser.getInstance().getLargeRes(textMessage.getBody());
            if (resId > 0) {
                ...
                /// M: add for iSMS, hide emoticon view
                mIpImageView.setVisibility(View.GONE);
                mIpEmoticonView.setVisibility(View.GONE);
...
//        SmileyParser parser = SmileyParser.getInstance();
        buf.append(com.android.mms.isms.util.message.SmileyParser.getInstance().addSmileySpans(body));
        return buf;
    }
    ...
    private void setIpImageItem(IpImageMessage imageMessage) {
        ...
        if (imageMessage.isInboxMsgDownloalable()) {
            ...
            if (MessageUtils.getMessageManager(this).isDownloading(msgId)) {
                ...
                int progress = MessageUtils.getMessageManager(this).getDownloadProcess(msgId);
                if (null != mImageDownloadProgressBar) {
            ...
            mActionButton.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if (MessageUtils.getMessageManager(DialogModeActivity.this).isDownloading(msgId)) {
                        MessageUtils.getMessageManager(DialogModeActivity.this).cancelDownloading(msgId);
                    } else {
                        MessageUtils.getMessageManager(DialogModeActivity.this).downloadAttach(msgId);
                    }
                }
            });
        ...
        if (TextUtils.isEmpty(imageMessage.getCaption())) {
            ...
            caption = com.android.mms.isms.util.message.SmileyParser.getInstance().addSmileySpans(imageMessage.getCaption());
            mCaption.setText(caption);
        }

        /// M: add for iSMS, hide text view
        mBodyTextView.setVisibility(View.GONE);
        mIpEmoticonView.setVisibility(View.GONE);
        /// M: add for iSMS, hide audio or vcard view
        mIpAudioView.setVisibility(View.GONE);
    ...
    private void setIpVoiceItem(IpVoiceMessage voiceMessage) {
        ...
        if (voiceMessage.isInboxMsgDownloalable()) {
            ...
            mAudioInfo.setVisibility(View.VISIBLE);
            mAudioInfo.setText(com.android.mms.isms.util.message.MessageUtils
                    .formatAudioTime(voiceMessage.getDuration()));
        }
        ...
        if (TextUtils.isEmpty(voiceMessage.getCaption())) {
            ...
            mCaption.setVisibility(View.VISIBLE);
            CharSequence caption = "";
            caption = com.android.mms.isms.util.message.SmileyParser.getInstance().addSmileySpans(voiceMessage.getCaption());
            mCaption.setText(caption);
        }

        /// M: add for iSMS, show audio view
        ...
        mBodyTextView.setVisibility(View.GONE);
        mIpEmoticonView.setVisibility(View.GONE);
        /// M: add for iSMS, hide image view or video view
        mIpImageView.setVisibility(View.GONE);
        /// M: add for iSMS, hide location view
        mIpLocationView.setVisibility(View.GONE);
    }
    ...
    private void setIpVideoItem(IpVideoMessage videoMessage) {
        ...
        if (TextUtils.isEmpty(videoMessage.getCaption())) {
            ...
            mCaption.setVisibility(View.VISIBLE);
            CharSequence caption = "";
            caption = com.android.mms.isms.util.message.SmileyParser.getInstance().addSmileySpans(videoMessage.getCaption());
            mCaption.setText(caption);
        }
    ...
    private boolean setPicView(String filePath) {
        ...
        if (bitmap != null) {
            bitmap = NmsCommonUtils.resizeImage(bitmap, mWidth, bitmap.getHeight() * mWidth / bitmap.getWidth(), true);
    ...
    private void sendIpTextMessage(String body, int simId, String to) {
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                MmsLog.d(TAG, "sendMessageForIsms(): calling API: saveIpMsg().");
                int ret = -1;
                ret = MessageUtils.getMessageManager(DialogModeActivity.this).saveIpMsg(ipMessage, sendMode);
                if (ret == -1) {
                    MmsLog.w(TAG, "sendMessageForIsms failed! ");
                } else {
                    // success.
                    onMessageSent();
                }
    ...
    private boolean isCurGroupIpMessage() {
        ...
        if (isCurIpMessage()) {
            ...
            if (conv != null) {
                ...
                if (number != null && number.startsWith(MessageUtils.GROUP_START_STR)) {
    ...
    private String getCurGroupIpMessageNumber() {
        ...
        if (isCurIpMessage()) {
            ...
            if (conv != null) {
                ...
                if (num != null && num.startsWith(MessageUtils.GROUP_START_STR)) {
                    // this is group message
                    number = num;
                }
            }
        ...
        if (isCurIpMessage()) {
            ...
            if (conv != null) {
                ...
                if (nam != null) {
                    ...
                    name = MessageUtils.getContactManager(this).getNameByThreadId(getThreadId());
                }
            }
        }
    ...
    public void notificationsReceived(Intent intent) {
        ...
        case MessageUtils.ISMS_IP_MESSAGE_STATUS_ACTION:
            try {
                if (MessageUtils.getActionTypeByAction(action) == MessageUtils.ISMS_DOWNLOAD_ATTACH_STATUS_ACTION) {
    ...
    private void openIsmsThread(final long threadId, boolean isImportant) {
