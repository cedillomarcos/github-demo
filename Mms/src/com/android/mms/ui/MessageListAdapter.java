...
public class MessageListAdapter extends MessageCursorAdapter {
    ...
    private static final String ISMS_TAG = "Mms/isms/MessageListAdapter";
    ...
    /// M: add for iSMS
    public static final int INCOMING_ITEM_TYPE_ISMS = 2;
    public static final int OUTGOING_ITEM_TYPE_ISMS = 3;
    /// M: add for iSMS
    private int[] layoutList = { R.layout.message_list_item_recv,R.layout.message_list_item_send
            , R.layout.message_list_item_recv_isms, R.layout.message_list_item_send_isms
            };
        ...
        if (view instanceof MessageListItem) {
            if (!mIsScrolling || mIsDeleteMode) {
                ...
                if (msgItem != null) {
                    ...
                    /// M: for multi-delete
                    if (mIsDeleteMode) {
                        /// M: fix bug ALPS00432495, catch
                        /// IllegalArgumentException for this special sms @{
                        if (msgItem.isSimMsg() && cursor.getColumnIndex("index_on_icc") != -1) {
                            ...
                            if (msgItem.isSimMsg() && cursor.getColumnIndex("index_on_icc") == -1) {
                                Log.w(TAG, "SimMsg but column 'index_on_icc' does not exist:");
                            }
    ...
    public View newView(Context context, Cursor cursor, ViewGroup parent) {
        ...
        case INCOMING_ITEM_TYPE_ISMS:
            return mInflater.inflate(R.layout.message_list_item_recv_isms, parent, false);
        case OUTGOING_ITEM_TYPE_ISMS:
            return mInflater.inflate(R.layout.message_list_item_send_isms, parent, false);
        default:
            return mInflater.inflate(R.layout.message_list_item_send, parent, false);
        }
    }
    ...
    public MessageItem getCachedMessageItem(String type, long msgId, Cursor c) {
        ...
        if (item == null && c != null && isCursorValid(c)) {
            /// M: add for iSMS, add isDrawTimeDivider status to MessageItem.
    ...
    public int getItemViewType(int position) {
        ...
        switch (getItemViewType(cursor)) {
        case INCOMING_ITEM_TYPE:
            MmsLog.d(ISMS_TAG, "getItemViewType(): item type = INCOMING_ITEM_TYPE");
            return INCOMING_ITEM_TYPE;
        case OUTGOING_ITEM_TYPE:
            MmsLog.d(ISMS_TAG, "getItemViewType(): item type = OUTGOING_ITEM_TYPE");
            return OUTGOING_ITEM_TYPE;
        case INCOMING_ITEM_TYPE_ISMS:
            MmsLog.d(ISMS_TAG, "getItemViewType(): item type = INCOMING_ITEM_TYPE_ISMS");
            return INCOMING_ITEM_TYPE_ISMS;
        case OUTGOING_ITEM_TYPE_ISMS:
            MmsLog.d(ISMS_TAG, "getItemViewType(): item type = OUTGOING_ITEM_TYPE_ISMS");
            return OUTGOING_ITEM_TYPE_ISMS;
        default:
            MmsLog.d(ISMS_TAG, "getItemViewType(): item type = INCOMING_ITEM_TYPE (default)");
            return INCOMING_ITEM_TYPE;
        }
    }
    ...
    private int getItemViewType(Cursor cursor) {
        String type = cursor.getString(mColumnsMap.mColumnMsgType);
        MmsLog.d(ISMS_TAG, "getItemViewType(): message type = " + type);
        ...
        if ("sms".equals(type)) {
            ...
            MmsLog.d(ISMS_TAG, "getItemViewType(): ipMessageId = " + ipMessageId);
            if (ipMessageId > 0 && !isSimMsg) {
                return boxId == Mms.MESSAGE_BOX_INBOX ? INCOMING_ITEM_TYPE_ISMS : OUTGOING_ITEM_TYPE_ISMS;
            }
        } else {
    ...
    /// M: add for iSMS, message time divider {@
    private boolean isMessageItemShowTimeDivider(Cursor cursor) {
        MmsLog.w(ISMS_TAG, "isMessageItemShowDivider(): cursor = " + cursor);
        if (null == cursor) {
            MmsLog.w(ISMS_TAG, "isMessageItemShowDivider(): cursor is null!");
            return false;
        }
        if (cursor.isBeforeFirst() || cursor.isAfterLast()) {
            MmsLog.w(ISMS_TAG, "isMessageItemShowDivider(): cursor is at before first or after last!");
            return false;
        }
        if (cursor.isFirst() || cursor.getCount() == 1) {
            MmsLog.w(ISMS_TAG, "isMessageItemShowDivider(): cursor is the first or cursor count is 1! Is first?="
        ...
        MmsLog.w(ISMS_TAG, "isMessageItemShowDivider(): currentMessageTime = " + sdf.format(new Date(currentMessageTime))
                + ", previousMessageTime = " + sdf.format(new Date(previousMessageTime)));
        return com.android.mms.isms.util.message.MessageUtils
                .shouldShowTimeDivider(previousMessageTime, currentMessageTime);
    }

    private long getMessageTimeByCursor(Cursor cursor) {
        MmsLog.w(ISMS_TAG, "getMessageTimeByCursor()");
        String type = cursor.getString(mColumnsMap.mColumnMsgType);
        if (type.equals("mms")) {
    ...
    private boolean getMessageImportatStatusByCursor(Cursor cursor) {
        MmsLog.w(ISMS_TAG, "getMessageImportatStatusByCursor()");
        if (null == cursor) {
            MmsLog.w(ISMS_TAG, "getMessageImportatStatusByCursor(): cursor is null!");
            return false;
        }
        if (cursor.isBeforeFirst() || cursor.isAfterLast()) {
            MmsLog.w(ISMS_TAG, "getMessageImportatStatusByCursor(): cursor is at before first or after last!");
            return false;
        }
        String type = cursor.getString(mColumnsMap.mColumnMsgType);
        if (type.equals("mms")) {
    ...
    private boolean getNextMessageUnreadStatusByCursor(Cursor cursor) {
        MmsLog.w(ISMS_TAG, "getNextMessageUnreadStatusByCursor()");
        if (null == cursor) {
            MmsLog.w(ISMS_TAG, "getNextMessageUnreadStatusByCursor(): cursor is null!");
            return false;
        }
        if (cursor.isBeforeFirst() || cursor.isAfterLast()) {
            MmsLog.w(ISMS_TAG, "getNextMessageUnreadStatusByCursor(): cursor is at before first or after last!");
            return false;
        }
//        if (cursor.isLast() || cursor.getCount() == 1) {
//            MmsLog.w(ISMS_TAG, "getNextMessageUnreadStatusByCursor(): cursor is the first or cursor count is 1!"
...
//        }
    ...
    /// M: add for iSMS, message unread divider {@
    private boolean isMessageItemShowUnreadDivider(Cursor cursor) {
        MmsLog.w(ISMS_TAG, "isMessageItemShowUnreadDivider(): cursor = " + cursor);
        MmsLog.w(TAG_DIVIDER, "adapter.isMessageItemShowUnreadDivider(): cursor = " + cursor);
        if (null == cursor) {
            MmsLog.w(ISMS_TAG, "isMessageItemShowUnreadDivider(): cursor is null!");
            MmsLog.w(TAG_DIVIDER, "adapter.isMessageItemShowUnreadDivider(): cursor is null!");
            return false;
        }
        if (cursor.isBeforeFirst() || cursor.isAfterLast()) {
            MmsLog.w(ISMS_TAG, "isMessageItemShowUnreadDivider(): cursor is at before first or after last!");
            MmsLog.w(TAG_DIVIDER, "adapter.isMessageItemShowUnreadDivider(): cursor is at before first or after last!");
            return false;
        }
        if (cursor.isFirst() || cursor.getCount() == 1) {
            MmsLog.w(ISMS_TAG, "isMessageItemShowUnreadDivider(): cursor is the first or cursor count is 1! Is first?="
        ...
        cursor.moveToNext();
        MmsLog.w(ISMS_TAG, "isMessageItemShowUnreadDivider(): currentMessageUnreadStatus = " + currentMessageUnreadStatus
    ...
    private boolean isMessageUnreadByCursor(Cursor cursor) {
        MmsLog.w(ISMS_TAG, "isMessageUnreadByCursor(): msgId = " + cursor.getInt(mColumnsMap.mColumnMsgId));
        MmsLog.w(TAG_DIVIDER, "adapter.isMessageUnreadByCursor(): msgId = " + cursor.getInt(mColumnsMap.mColumnMsgId));
        String type = cursor.getString(mColumnsMap.mColumnMsgType);
        if (type.equals("mms")) {
            MmsLog.w(ISMS_TAG, "isMessageUnreadByCursor(): read = " + cursor.getInt(mColumnsMap.mColumnMmsRead));
            ...
            MmsLog.w(ISMS_TAG, "isMessageUnreadByCursor(): read = " + cursor.getInt(mColumnsMap.mColumnSmsRead));
    ...
    private boolean isMessageItemShowOnlineDivider(Cursor cursor) {
        MmsLog.w(ISMS_TAG, "isMessageItemShowOnlineDivider(): cursor = " + cursor);
        MmsLog.w(TAG_DIVIDER, "adapter.isMessageItemShowOnlineDivider(): cursor = " + cursor);
        if (null == cursor) {
            MmsLog.w(ISMS_TAG, "isMessageItemShowOnlineDivider(): cursor is null!");
            MmsLog.w(TAG_DIVIDER, "adapter.isMessageItemShowOnlineDivider(): cursor is null!");
            return false;
        }
        if (cursor.isBeforeFirst() || cursor.isAfterLast()) {
            MmsLog.w(ISMS_TAG, "isMessageItemShowOnlineDivider(): cursor is at before first or after last!");
    ...
    private boolean isMessageItemShowLoadAllButton(Cursor cursor) {
        MmsLog.d(ISMS_TAG, "isMessageItemShowLoadAllButton(): cursor = " + cursor);
        MmsLog.d(TAG_DIVIDER, "adapter.isMessageItemShowLoadAllButton(): cursor = " + cursor);
        if (null == cursor) {
            MmsLog.w(ISMS_TAG, "isMessageItemShowLoadAllButton(): cursor is null!");
            MmsLog.w(TAG_DIVIDER, "adapter.isMessageItemShowLoadAllButton(): cursor is null!");
            return false;
        }
        if (cursor.isBeforeFirst() || cursor.isAfterLast()) {
            MmsLog.w(ISMS_TAG, "isMessageItemShowLoadAllButton(): cursor is at before first or after last!");
