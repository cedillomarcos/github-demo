...
public class ConversationList extends ListActivity implements DraftCache.OnDraftChangedListener,
    ...
    /// @}
    /// M: Code analyze 019, For new feature ALPS00101272, Applaunch time auto test . @{
    ...
    /// M: add for iSMS {@
    private static final String ISMS_TAG = "Mms/isms/ConvList";
    ...
    private NmsConversationEmptyView mEmptyView;
    ...
    private int mQueryCount = 0;           //The count of doing query.
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mEmptyView = (NmsConversationEmptyView)findViewById(R.id.empty2);
        ...
        mNetworkStatusBar = (LinearLayout)findViewById(R.id.noinernet);
    ...
    private final ConversationListAdapter.OnContentChangedListener mContentChangedListener =
        ...
        /// M: Remove cursor leak and reduce needless query @{
            MmsLog.d(TAG, "onContentChanged: mIsInActivity=" + mIsInActivity +
                ",mQueryCount=" + mQueryCount + ",mNeedQuery=" + mNeedQuery);
            /* Only need when activity is shown*/
            if (mIsInActivity) {
                mNeedQuery = true;
                /* If there is any doing query, wait its finish to do this new query*/
                if (mQueryCount == 0) {
                    startAsyncQuery();
                }
            }
        ...
        try {
            ...
            if (MmsConfig.isActivated(this)) {
                ...
                case OPTION_CONVERSATION_LIST_ALL:
                    MmsLog.d(ISMS_TAG, "startAsyncQuery(): query for all messages except spam");
                    mSpinnerTextView.setText(R.string.ipmsg_conversation_list_all);
                    selection = "threads._id not in (SELECT DISTINCT "
                                + Sms.THREAD_ID
                                + " FROM thread_settings WHERE spam=1) ";
                    Conversation.startQueryIsms(mQueryHandler, THREAD_LIST_QUERY_TOKEN, selection);
                    mQueryCount++;
                ...
                case OPTION_CONVERSATION_LIST_IMPORTANT:
                    mSpinnerTextView.setText(R.string.ipmsg_conversation_list_important);
                    ...
                    MmsLog.d(ISMS_TAG, "startAsyncQuery(): query for important messages, selection = " + selection);
                    Conversation.startQueryIsms(mQueryHandler, THREAD_LIST_QUERY_TOKEN, selection);
                    mQueryCount++;
                    Conversation.startQuery(mQueryHandler, UNREAD_THREADS_QUERY_TOKEN, Threads.READ + "=0"
                            + " and " + selection);
                ...
                case OPTION_CONVERSATION_LIST_GROUP_CHATS:
                    MmsLog.d(ISMS_TAG, "startAsyncQuery(): query for group messages");
                    mSpinnerTextView.setText(R.string.ipmsg_conversation_list_group_chats);
                    selection = "threads._id IN (SELECT DISTINCT " + Sms.THREAD_ID
                            ...
                            + "SUBSTR(address, 1, 4) = '" + MessageUtils.GROUP_START_STR + "'" + ")";
                ...
                case OPTION_CONVERSATION_LIST_SPAM:
                    mSpinnerTextView.setText(R.string.ipmsg_conversation_list_spam);
                    ...
                    MmsLog.d(ISMS_TAG, "startAsyncQuery(): query for spam messages, selection = " + selection);
                    Conversation.startQueryIsms(mQueryHandler, THREAD_LIST_QUERY_TOKEN, selection);
                    mQueryCount++;
                    Conversation.startQuery(mQueryHandler, UNREAD_THREADS_QUERY_TOKEN, Threads.READ + "=0"
                            + " and " + selection);
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        if (MmsConfig.isActivated(this)) {
            ...
            if (item != null &&
                MessageUtils.getServiceManager(this).isFeatureSupported(IpMessageConsts.FeatureId.GROUP_MESSAGE)) {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (MmsConfig.isActivated(this)) {
            ...
            case R.id.create_group_chat:
                if (MessageUtils.checkCurrentIpMessageServiceStatus(this, true, null)) {
                    ...
                    MessageUtils.startRemoteActivity(this, createGroupIntent);
                } else {
                    return true;
                }
                break;
            case R.id.send_invitations:
                ...
                MessageUtils.startRemoteActivity(this, createGroupIntent2);
                break;
            default:
                break;
            }
    ...
    protected void onListItemClick(ListView l, View v, int position, long id) {
        ...
        if (MmsConfig.getIpMessagServiceId(this) > IpMessageServiceId.NO_SERVICE) {
            ...
            if (conv.getType() == Threads.IP_MESSAGE_GUIDE_THREAD) {
                ...
                MessageUtils.startRemoteActivity(this, it);
                conv.markAsRead();
                return;
            }
            /// M: add for iSMS, handle group thread
            ...
            MmsLog.i(ISMS_TAG, "open thread by number " + number);
            if (conv.getRecipients().size() == 1 && number.startsWith(MessageUtils.GROUP_START_STR)) {
                MmsLog.i(ISMS_TAG, "open group thread by thread id " + tid);
                conv.markAsSeen();
                if (sConversationListOption == OPTION_CONVERSATION_LIST_IMPORTANT) {
                    openIsmsThread(tid, true);
                } else {
                    openIsmsThread(tid, false);
                }
                return;
            }
            /// M: add for iSMS, handle important thread
            if (sConversationListOption == OPTION_CONVERSATION_LIST_IMPORTANT) {
                MmsLog.i(ISMS_TAG, "open important thread by thread id " + tid);
    ...
    public static void confirmDeleteThreadDialog(final DeleteThreadListener listener,
        ...
        if (!hasLockedMessages) {
            ...
            if (MmsConfig.isActivated(context)) {
                checkbox.setText(R.string.ip_message_delete_important);
            }
        ...
        public void onClick(DialogInterface dialog, final int whichButton) {
            MessageUtils.handleReadReport(mContext, mThreadIds,
                ...
                public void run() {
                    ...
                    /// M: delete ipmessage in isms db
                    MessageUtils.deleteIpMessage(mContext, mThreadIds, mDeleteLockedMessages, mMaxSmsId);
...
//        protected void myonQueryComplete(int token, Object cookie, Cursor cursor) {
        ...
        protected void onQueryComplete(int token, Object cookie, Cursor cursor) {
              /// M: Code analyze 015, For bug,  add cursor == null check . @{
              MmsLog.d(TAG, "onQueryComplete mNeedQuery = " + mNeedQuery + " mIsInActivity = " + mIsInActivity + " mQueryCount = " + mQueryCount);
              if (cursor == null) {
              /// M: Decrease query counter and do next query if any request  @{
                  ...
                  if (token == THREAD_LIST_QUERY_TOKEN || token == UNREAD_THREADS_QUERY_TOKEN) {
                      MmsLog.d(TAG, "onQueryComplete cursor == null mQueryCount =" + mQueryCount);
                      if (mQueryCount > 0) {
                        mQueryCount--;
                      }
                  }
                  if (mNeedQuery && mIsInActivity && mQueryCount == 0) {
            ...
            /// M: If no listener for content change, means no need to refresh list @{
                MmsLog.d(TAG, "onQueryComplete THREAD_LIST_QUERY_TOKEN mQueryCount = " + mQueryCount);
                if (mQueryCount > 0) {
                    mQueryCount--;
                }
            ...
            /// M: @{
                MmsLog.d(TAG, "onQueryComplete UNREAD_THREADS_QUERY_TOKEN mQueryCount = " + mQueryCount);
                if (mQueryCount > 0) {
                    mQueryCount--;
                }
            /// @}
        ...
        /// M: Do next query if any requested @{
            MmsLog.d(TAG, "onQueryComplete end " + mNeedQuery + "," + mIsInActivity + "," + mQueryCount);
            if (mNeedQuery && mIsInActivity && (mQueryCount == 0)) {
                startAsyncQuery();
            }
        /// @}
    ...
    private class ModeCallback implements ActionMode.Callback {
        ...
        public boolean onPrepareActionMode(ActionMode mode, Menu menu) {
            ...
            if (!MessageUtils.getServiceManager(ConversationList.this).isFeatureSupported(
        ...
        public boolean onActionItemClicked(final ActionMode mode, MenuItem item) {
            ...
            case R.id.export:
                if (mSelectedThreadIds.size() > 0) {
                    ...
                    // TODO iSMS, show dialog for download to SD card or share via Email
            ...
            case R.id.mark_as_spam:
                        ...
                        new Thread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                for (Long threadId : threadIds2) {
                                    ...
                                    int contactId = MessageUtils.getContactManager(ConversationList.this)
                                        .getContactIdByNumber(numbers);
                                    contactIds[i] = contactId;
                                    i++;
                                    MmsLog.d(TAG, "threadId:" + threadId + ", contactId:" + contactId);
                                }
                                MessageUtils.getContactManager(ConversationList.this).addContactToSpamList(contactIds);
                            }
                        }).start();
                        mode.finish();
                    }
                ...
                builder.setTitle(R.string.mark_as_spam)
                    ...
                    .setMessage(R.string.mark_as_spam_tips)
                    .show();
                break;
            case R.id.mark_as_nonspam:
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        MessageUtils.getContactManager(ConversationList.this).deleteContactFromSpamList(contactIds);
                    }
                }).start();
                mode.finish();
                break;
        ...
        private void updateActionMode() {
            /// M: Code analyze 018, For bug, enable or diable deleteitem menu . @{
            if (deleteitem != null) {
                if (mCheckedNum > 0) {
    ...
    private class PostDrawListener implements android.view.ViewTreeObserver.OnPostDrawListener {
        ...
        public boolean onPostDraw() {
            MmsLog.i("AppLaunch", "[AppLaunch] MMS onPostDraw");
            return true;
        }
    }
    /// @}
    ...
    protected void onResume() {
        ...
        IAppGuideExt appGuideExt = (IAppGuideExt)MmsPluginManager.getMmsPluginObject(MmsPluginManager.MMS_PLUGIN_TYPE_APPLICATION_GUIDE);
    ...
    private void initPlugin(Context context){
        try {
            mMmsConversationPlugin = (IMmsConversation)EncapsulatedPluginManager.createPluginObject(context, IMmsConversation.class.getName());
    ...
    /// M: add for iSMS: important, spam, group chats {@
    ...
    private ArrayAdapter<String> getDropDownMenuData(ArrayAdapter<String> adapter, int dropdownStatus) {
        ...
        if (dropdownStatus != OPTION_CONVERSATION_LIST_ALL) {
            adapter.add(res.getText(R.string.ipmsg_conversation_list_all).toString());
        }

        if (dropdownStatus != OPTION_CONVERSATION_LIST_IMPORTANT) {
            adapter.add(res.getText(R.string.ipmsg_conversation_list_important).toString());
        }

        if (dropdownStatus != OPTION_CONVERSATION_LIST_GROUP_CHATS) {
            adapter.add(res.getText(R.string.ipmsg_conversation_list_group_chats).toString());
        }

        if (dropdownStatus != OPTION_CONVERSATION_LIST_SPAM) {
            adapter.add(res.getText(R.string.ipmsg_conversation_list_spam).toString());
        }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        MmsLog.d(ISMS_TAG, "onActivityResult(): requestCode=" + requestCode + ", resultCode=" + resultCode);
        if (resultCode != RESULT_OK) {
            MmsLog.d(ISMS_TAG, "onActivityResult(): result is not OK.");
            return;
        }
        switch (requestCode) {
        case REQUEST_CODE_SELECT_CONTACT_FOR_GROUP:
            String[] mSelectContactsIds = data.getStringArrayExtra(MessageUtils.SELECTION_CONTACT_RESULT);
            if (mSelectContactsIds != null) {
                for (String contactId : mSelectContactsIds) {
                    MmsLog.d(ISMS_TAG, "onActivityResult(): SELECT_CONTACT get contact id = " + contactId);
                }
        ...
        case REQUEST_CODE_INVITE:
            ...
            if (mSelectContactsNumbers != null) {
                MmsLog.d(ISMS_TAG, "mSelectContactsNumbers:" + mSelectContactsNumbers);
                StringBuilder numberString = new StringBuilder();
                Intent it = new Intent(this, ComposeMessageActivity.class);
                it.setAction(Intent.ACTION_SENDTO);
                Uri uri = Uri.parse("smsto:" + mSelectContactsNumbers);
                it.setData(uri);
                it.putExtra("sms_body", getString(R.string.invite_friends_to_ipmsg_content));
                startActivity(it);
            } else {
                MmsLog.d(ISMS_TAG, "onActivityResult(): INVITE get contact id is NULL!");
            }
            break;
        default:
            MmsLog.d(ISMS_TAG, "onActivityResult(): default return.");
            return;
        }
    }

    public void openIsmsThread(final long threadId, boolean isImportant) {
    ...
    public void notificationsReceived(Intent intent) {
        MmsLog.d(MessageUtils.ISMS_NOTIFICATION_TAG, "convList.notificationsReceived(): start, intent = " + intent);
...
//            public static final int OFF = 0;
        ...
        case MessageUtils.ISMS_IM_STATUS_ACTION:
            ...
            int status = MessageUtils.getContactManager(this).getStatusByNumber(number);
            ...
            //long threadId = conv.getThreadId();
            //MmsLog.d(MessageUtils.ISMS_NOTIFICATION_TAG, "number query threadId:" + threadId);
            switch (status) {
            case ContactStatus.TYPING:
                conv.setTyping(true);
                MmsLog.d(MessageUtils.ISMS_NOTIFICATION_TAG, "start typing");
                mTypingCounter++;
                runOnUiThread(new Runnable() {
                    public void run() {
                        mListAdapter.notifyDataSetChanged();
            ...
            case ContactStatus.STOP_TYPING:
                ...
                MmsLog.d(MessageUtils.ISMS_NOTIFICATION_TAG, "stop typing");
                mTypingCounter--;
                runOnUiThread(new Runnable() {
                    public void run() {
                        mListAdapter.notifyDataSetChanged();
                ...
                break;
            default:
                MmsLog.d(MessageUtils.ISMS_NOTIFICATION_TAG, "ignore a status:" + status);
                break;
            }
            */
            break;
            ...
            if (!mIsSendEmail) { // it is saving
                    ...
                    runOnUiThread(new Runnable() {
                        public void run() {
                            Toast.makeText(getApplicationContext(),
                                getResources().getString(R.string.save_chat_history_failed), Toast.LENGTH_SHORT).show();
                        }
                    });
                }
            } else {
                ...
                if (done == 0 && filePath != null) {
                    ...
                    runOnUiThread(new Runnable() {
                        public void run() {
                            Toast.makeText(getApplicationContext(),
                                getResources().getString(R.string.send_chat_history_failed), Toast.LENGTH_SHORT).show();
                        }
                    });
                }
            }
        ...
        case MessageUtils.ISMS_RESTORE_MSG_STATUS_ACTION:
            break;
        case MessageUtils.ISMS_UPDATE_GROUP_INFO:
            int groupId = intent.getIntExtra(IpMessageConsts.UpdateGroup.GROUP_ID, -1);
            MmsLog.d(TAG, "update group info,group id:" + groupId);
            String number = MessageUtils.getContactManager(this).getNumberByEngineId((short)groupId);
            ...
            break;
        case MessageUtils.ISMS_IPMESSAGE_CONTACT_UPDATE:
             ...
             *  we will not receive a ISMS_UPDATE_GROUP_INFO event,
    ...
    private void showExportDialog(final HashSet<Long> selectedThreadIds, final ActionMode mode) {
        ...
        urls.add(getString(R.string.dialog_save_title));
        urls.add(getString(R.string.dialog_email_title));
            ...
            public final void onClick(DialogInterface dialog, int which) {
                if (!Environment.getExternalStorageState().equals(android.os.Environment.MEDIA_MOUNTED)) {
                    Toast.makeText(ConversationList.this,
                        ConversationList.this.getResources().getString(R.string.no_sdcard_suggestion), Toast.LENGTH_LONG).show();
                    dialog.dismiss();
                    return;
                }
                switch (which) {
                ...
                case 1: // email
                    ...
                    int id = R.string.chat_setting_saving;
                    if (which == 1) {
                        id = R.string.chat_setting_sending;
                        mIsSendEmail = true;
                    }
                    mSaveChatHistory = ProgressDialog.show(ConversationList.this, null,
                            getResources().getString(id));
                    MessageUtils.getChatManager(ConversationList.this).saveChatHistory(selectThreadIds);
                    break;
                default:
                    break;
                }
