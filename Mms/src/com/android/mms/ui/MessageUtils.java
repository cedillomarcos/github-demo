...
public class MessageUtils {
    ...
    /// M: add for iSMS, iSMS tag
    private static final String ISMS_TAG = "Mms/isms";
    ...
    private static String getTextMessageDetails(Context context, MessageItem msgItem) {
        ...
        if (msgItem.mIpMessageId > 0) {
            details.append(res.getString(R.string.ipmsg_ipmsg));
        } else {
            details.append(res.getString(R.string.text_message));
        }
...
//            handler.postDelayed(showProgress, 1000);
    ...
    public static boolean isLocalNumber(String number) {
        ...
        if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {
            return PhoneNumberUtils.compare(number, getLocalNumberGemini(EncapsulatedPhone.GEMINI_SIM_1))
                   || PhoneNumberUtils.compare(number, getLocalNumberGemini(EncapsulatedPhone.GEMINI_SIM_2));
        } else {
            return PhoneNumberUtils.compare(number, getLocalNumber());
        }
        /// @}
...
//                    launchSlideshowActivity(activity, msgUri, requestCode);
    ...
    public static String getLocalNumberGemini(int simId) {
        ...
        if (EncapsulatedPhone.GEMINI_SIM_1 == slotId) {
            if (null == sLocalNumber) {
                sLocalNumber = teleManager.getLine1Number(slotId);
       }
            MmsLog.d(MmsApp.TXN_TAG, "getLocalNumberGemini, Sim ID=" + simId + ", slot ID=" + slotId +", lineNumber=" + sLocalNumber);

            return sLocalNumber;
        } else if (EncapsulatedPhone.GEMINI_SIM_2 == slotId) {
            if (null == sLocalNumber2) {
                sLocalNumber2 = teleManager.getLine1Number(slotId);
            }
            MmsLog.d(MmsApp.TXN_TAG, "getLocalNumberGemini, Sim ID=" + simId + ", slot ID=" + slotId +", lineNumber=" + sLocalNumber);
    ...
    public final static Map<Integer, CharSequence> simInfoMap = new HashMap<Integer, CharSequence>();
    ...
    public static CharSequence getSimInfoSync(Context context, int simId){
        ...
        if(null != simInfo){
            ...
            int colorRes = (simInfo.getSlot() >= 0) ? simInfo.getSimBackgroundLightRes() : EncapsulatedR.drawable.sim_background_locked;
            Drawable drawable = context.getResources().getDrawable(colorRes);
            buf.setSpan(new EncapsulatedBackgroundImageSpan(colorRes, drawable), 0, buf.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            //set simInfo color
...
//        final long totalBlocks = stat.getBlockCount();
    ...
    public static boolean existingExternalSD(EncapsulatedStorageManager storageManager) {
        ...
        if (VolumeState.equals(Environment.MEDIA_MOUNTED)) {
            return true;
        } else {
            return false;
        }
            ...
            public void run() {
                ...
                for (Long threadId : threadIds) {
                    ...
                    if (MmsConfig.getIpMessagServiceId(context) == IpMessageServiceId.ISMS_SERVICE &&
                        ...
                        if (number.startsWith(MessageUtils.GROUP_START_STR)) {
                    ...
                    if (conv.getType() != 10) {
                        ...
                        intent.putExtra(Intent.EXTRA_SHORTCUT_ICON_RESOURCE,
                                Intent.ShortcutIconResource.fromContext(context, R.drawable.ic_launcher_smsmms));
                        Intent it = createIntentByThreadId(context, threadId);
                        it.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK
                                        | Intent.FLAG_ACTIVITY_SINGLE_TOP
                                        | Intent.FLAG_ACTIVITY_CLEAR_TOP);
                        intent.putExtra(Intent.EXTRA_SHORTCUT_INTENT, it);
                        ...
                        intent.putExtra(EXTRA_SHORTCUT_STEP_NUMBER, currentNumber);
                        context.sendBroadcast(intent);
                    } else {// this is a guide thread
                        MmsLog.d("shortcut", "create guide shortcut.");
                        intent = new Intent(INSTALL_SHORTCUT);
                        intent.putExtra(Intent.EXTRA_SHORTCUT_NAME, context.getString(R.string.str_ipmsg_service_title));
                        intent.putExtra(Intent.EXTRA_SHORTCUT_ICON_RESOURCE,
                                Intent.ShortcutIconResource.fromContext(context, R.drawable.ic_launcher_smsmms));
    ...
    public static Intent createIntentByThreadId(Context context, long threadId) {
    ...
    /// M: add for iSMS {@
    public static IIpMessagePlugin mIpMessagePlugin = null;
    public static final String GROUP_START_STR = "7---";
    public static final String SELECTION_CONTACT_RESULT = "contactId";
    public static final String ISMS_NOTIFICATION_TAG = "Mms/noti";
    public static final String WALLPAPER_PATH = "/data/data/com.android.providers.telephony/app_wallpaper";

    public synchronized static IIpMessagePlugin getIpMessagePlugin(Context context) {
        if (mIpMessagePlugin == null) {
            initIpMessagePlugin(context);
        }
        return mIpMessagePlugin;
    }

    private synchronized static void initIpMessagePlugin(Context context) {
        if (mIpMessagePlugin == null) {
            try {
                mIpMessagePlugin = (IIpMessagePlugin) PluginManager.createPluginObject(context,
                        IIpMessagePlugin.class.getName(), PLUGIN_VERSION, PLUGIN_METANAME);
                MmsLog.d(TAG, "mIpMessagePlugin = " + mIpMessagePlugin);
            } catch (Plugin.ObjectCreationException e) {
                mIpMessagePlugin = new IpMessagePluginImpl(context);
                MmsLog.d(TAG, "default mIpMessagePlugin = " + mIpMessagePlugin);
            }
        }
    }
     ...
     */
    private synchronized static ActivitiesManager getActivitiesManager(Context context) {
     ...
     * @return ChatManager
     */
    public synchronized static ChatManager getChatManager(Context context) {
     ...
     */
    public synchronized static ContactManager getContactManager(Context context) {
     ...
     */
    public synchronized static NotificationsManager getNotificationsManager(Context context) {
     ...
     * @return GroupManager
     */
    public synchronized static GroupManager getGroupManager(Context context) {
     ...
     */
    public synchronized static MessageManager getMessageManager(Context context) {
     ...
     */
    public synchronized static com.mediatek.mms.ipmessage.ServiceManager getServiceManager(Context context) {
     ...
     */
    public synchronized static com.mediatek.mms.ipmessage.ResourceManager getResourceManager(Context context) {
     ...
     */
    public synchronized static SettingsManager getSettingsManager(Context context) {
    ...
    public static final int ISMS_ERROR_ACTION                  = 0;
    public static final int ISMS_NEW_MESSAGE_ACTION            = 1;
    public static final int ISMS_REFRESH_CONTACT_LIST_ACTION   = 2;
    public static final int ISMS_REFRESH_GROUP_LIST_ACTION     = 3;
    public static final int ISMS_SERCIVE_STATUS_ACTION         = 4;
    public static final int ISMS_IM_STATUS_ACTION              = 5;
    public static final int ISMS_SAVE_HISTORY_ACTION           = 6;
    public static final int ISMS_ACTIVATION_STATUS_ACTION      = 7;
    public static final int ISMS_IP_MESSAGE_STATUS_ACTION      = 8;
    public static final int ISMS_DOWNLOAD_ATTACH_STATUS_ACTION = 9;
    public static final int ISMS_SET_PROFILE_RESULT_ACTION     = 10;
    public static final int ISMS_BACKUP_MSG_STATUS_ACTION      = 11;
    public static final int ISMS_RESTORE_MSG_STATUS_ACTION     = 12;
    public static final int ISMS_UPDATE_GROUP_INFO             = 13;
    public static final int ISMS_IPMESSAGE_CONTACT_UPDATE      = 14;
    ...
    public static void removeIsmsNotificationListeners(Context context, INotificationsListener notiListener) {
        getIpMessagePlugin(context.getApplicationContext()).getNotificationsManager(context.getApplicationContext())
    ...
    public static void showInviteIpMsgConfirmDialog(Context context,
        ...
        new AlertDialog.Builder(context)
                .setIconAttribute(android.R.attr.alertDialogIcon)
                .setTitle(R.string.discard_message)
                .setMessage(R.string.invite_friends_to_ipmsg_dialog_msg)
                .setPositiveButton(R.string.yes, listener)
                .setNegativeButton(R.string.no, null)
                .show();
    }
    ...
    private static AlertDialog mActivationDialog = null;
    ...
    public static boolean checkCurrentIpMessageServiceStatus(final Activity activityContext, final boolean allowSwitchCard,
        ...
        MmsLog.d(ISMS_TAG, "checkCurrentIpMessageServiceStatus(): context = " + activityContext);
        if (activityContext == null) {
            MmsLog.e(ISMS_TAG, "checkCurrentIpMessageServiceStatus(): context = " + activityContext, new Exception());
            return false;
        }
        if (MmsConfig.getIpMessagServiceId(activityContext) == IpMessageServiceId.NO_SERVICE) {
            MmsLog.d(ISMS_TAG, "checkCurrentIpMessageServiceStatus(): service is not exist. return false!");
            return false;
        }
        ...
        MmsLog.d(ISMS_TAG, "checkCurrentIpMessageServiceStatus(): isServiceEnabled = " + isServiceEnabled
            + ", isServiceActivated = " + isServiceActivated);
        if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {
            MmsLog.d(ISMS_TAG, "checkCurrentIpMessageServiceStatus(): Dual SIM.");
            ...
            if (simInfoList.size() == 0) {
                ...
                MmsLog.d(ISMS_TAG, "checkCurrentIpMessageServiceStatus(): No SIM card!");
                toastLongMessage(R.string.no_sim_card);
            } else if (simInfoList.size() == 1) {
                /// M: add for IP message, one SIM
                MmsLog.d(ISMS_TAG, "checkCurrentIpMessageServiceStatus(): One SIM card!");
                if (isServiceEnabled) {
                    return true;
                ...
                MmsLog.d(ISMS_TAG, "checkCurrentIpMessageServiceStatus(): Two SIM card!");
                ...
                if (defaultSimId != Settings.System.DEFAULT_SIM_SETTING_ALWAYS_ASK
                    ...
                    if (isServiceEnabled && ipMsgHandler == null) {
                     ...
                     // one SIM didn't activated, one SIM disabled
                        boolean isSimActivated_0 = MmsConfig.isActivated(activityContext, (int) simInfoList.get(0).getSimId());
                        boolean isSimActivated_1 = MmsConfig.isActivated(activityContext, (int) simInfoList.get(1).getSimId());
                        if (isSimActivated_0 && isSimActivated_1) {
                            showActivateOrEnableIpMessageServiceDialog(activityContext, simInfoList,
                                DUAL_SIM, SIM_SELECTION_MODE_ENABLED, ipMsgHandler);
                        } else if (isSimActivated_0) {
                            showActivateOrEnableIpMessageServiceDialog(activityContext, simInfoList,
                                (int) simInfoList.get(0).getSimId(), SIM_SELECTION_MODE_ENABLED, ipMsgHandler);
                        } else if (isSimActivated_1) {
                            showActivateOrEnableIpMessageServiceDialog(activityContext, simInfoList,
            ...
            MmsLog.d(ISMS_TAG, "checkCurrentIpMessageServiceStatus(): Single SIM.");
...
//            ITelephony phone = ITelephony.Stub.asInterface(ServiceManager.checkService("phone"));
            ...
            if (phone != null) {
                try {
                    ...
                    MmsLog.e(ISMS_TAG, "check sim insert status failed");
                }
            }
            if (isServiceEnabled) {
                return true;
    ...
    private static void showActivitionDlg(final Activity activityContext, int mode, final int simId) {
        MmsLog.d(ISMS_TAG, "showActivitionDlg(): context = " + activityContext + ", mode = " + mode
            + ", simId = " + simId);
        ...
        String termContent = activityContext.getString(WELCOME == mode ? R.string.str_ipmsg_term_warn_welcome
                : R.string.str_ipmsg_term_warn_activate);
        SpannableString ss = new SpannableString(termContent);
        ss.setSpan(new URLSpan("noting"), termContent.indexOf(activityContext.getString(R.string.str_ipmsg_term_key)),
        ...
        mActivationDialog = new AlertDialog.Builder(activityContext)
            .setTitle(WELCOME == mode ? R.string.str_ipmsg_welcome_active : R.string.str_ipmsg_active)
            .setView(view)
            .setPositiveButton(R.string.str_ipmsg_agree_and_continue, new DialogInterface.OnClickListener() {
                public void onClick(DialogInterface dialog, int whichButton) {
                    MmsLog.d(ISMS_TAG, "showActivitionDlg(): agree and continue, simId = " + simId);
                    if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {
            ...
            .setOnCancelListener(new OnCancelListener() {
                @Override
                public void onCancel(DialogInterface dialog) {
                    MmsLog.d(ISMS_TAG, "showActivitionDlg(): cancel.");
                }
            })
            .create();
    ...
    private static class MyURLSpan extends ClickableSpan {
        ...
        public void onClick(View widget) {
            MmsLog.d(ISMS_TAG, "MyURLSpan.onClick(): context = " + mActivity + ", simId = " + mSimId);
            ...
            if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {
                MmsLog.d(ISMS_TAG, "MyURLSpan.onClick(): dual SIM");
                intent.putExtra(RemoteActivities.KEY_SIM_ID, mSimId);
            } else {
                MmsLog.d(ISMS_TAG, "MyURLSpan.onClick(): single SIM");
                intent.putExtra(RemoteActivities.KEY_SIM_ID, SpecialSimId.ALL_SIM_ID);
            }
            MmsLog.d(ISMS_TAG, "MyURLSpan.onClick(): start term activity. action = " + RemoteActivities.TERM
                + ", simId = " + mSimId);
    ...
    private static void showSimSelectionDialog(final Activity activityContext, List<SIMInfo> simInfoList,
        ...
        MmsLog.d(ISMS_TAG, "showSimSelectionDialogForActivation(): context = " + activityContext + ", mode = " + mode);
        if (simInfoList.size() == 0) {
            MmsLog.d(ISMS_TAG, "showSimSelectionDialogForActivation(): No SIM card!");
            toastLongMessage(R.string.no_sim_card);
            return;
        } else if (simInfoList.size() == 1) {
            MmsLog.d(ISMS_TAG, "showSimSelectionDialogForActivation(): One SIM card!");
            Intent intent = new Intent(RemoteActivities.ACTIVITION);
            intent.putExtra(RemoteActivities.KEY_SIM_ID, (int) simInfoList.get(0).getSimId());
            startRemoteActivity(activityContext, intent);
            return;
        }
        MmsLog.d(ISMS_TAG, "showSimSelectionDialogForActivation(): Two SIM cards! Show SIM selection dialog.");
        ...
        for (int i = 0; i < simInfoList.size(); i++) {
            ...
            int state = getSimStatus(i, simInfoList, EncapsulatedTelephonyManagerEx.getDefault());
        ...
        final SimpleAdapter a = new SimpleAdapter(activityContext, entries, R.layout.sim_selector,
        ...
        b.setTitle(mode == SIM_SELECTION_MODE_ACTIVATION ? R.string.sim_selected_dialog_title_for_activate :
            R.string.sim_selected_dialog_title_for_enable);
        ...
        b.setAdapter(a, new DialogInterface.OnClickListener() {
            ...
            public final void onClick(DialogInterface dialog, int which) {
                ...
                case SIM_SELECTION_MODE_ENABLED:
                    ...
                    toastLongMessage(R.string.ip_message_current_sim_enabled);
                    if (ipMsgHandler != null) {
                        Message message = new Message();
                        message.what = ComposeMessageActivity.UPDATE_SENDBUTTON;
    ...
    private static void showActivateOrEnableIpMessageServiceDialog(final Activity activityContext,
        ...
        MmsLog.d(ISMS_TAG, "showEnableIpMessageServiceDialog(): context = " + activityContext
            + ", simInfoList = " + simInfoList + ", simId = " + simId);
        int titleResId = mode == SIM_SELECTION_MODE_ACTIVATION ? R.string.ip_message_activate_title :
            R.string.ip_message_enable_title;
        int messageResId = mode == SIM_SELECTION_MODE_ACTIVATION ? R.string.ip_message_activate_message :
            R.string.ip_message_enable_message;
    ...
    private static void showSwitchSimDialog(final Activity activityContext, final long simId) {
        MmsLog.d(ISMS_TAG, "showSwitchSimDialog(): context = " + activityContext + ", simId = " + simId);
        AlertDialog switchDialog = new AlertDialog.Builder(activityContext)
            .setTitle(R.string.ip_message_switch_sim_title)
            .setMessage(R.string.ip_message_switch_sim_message)
            .setPositiveButton(R.string.ip_message_switch_sim_button, new DialogInterface.OnClickListener() {
                public void onClick(DialogInterface dialog, int whichButton) {
                    Settings.System.putLong(activityContext.getContentResolver(), Settings.System.SMS_SIM_SETTING, simId);
                    ...
                    toastLongMessage(R.string.ip_message_switch_sim_successfully);
                }
            })
            .setOnCancelListener(new OnCancelListener() {
                @Override
    ...
    private static void toastLongMessage(int resId) {
        MmsLog.d(ISMS_TAG, "toastLongMessage(): resId = " + resId);
        Toast.makeText(MmsApp.getApplication(), resId, Toast.LENGTH_LONG).show();
    }

    private static void toastShortMessage(int resId) {
        MmsLog.d(ISMS_TAG, "toastShortMessage(): resId = " + resId);
        Toast.makeText(MmsApp.getApplication(), resId, Toast.LENGTH_LONG).show();
    }
    ...
    public static boolean isHome(){
        ...
        ActivityManager activityManager = (ActivityManager)MmsApp.getApplication().getSystemService(Activity.ACTIVITY_SERVICE);
    ...
    public static void deleteIpMessage(Context ct, Collection<Long> threads, boolean delImportant, int maxSmsId) {
        ...
        if (threads == null) { // delete all ipmessage
            // no more constraint
        } else {
            if (threads.size() < 1) {
        ...
        MessageUtils.getMessageManager(ct).deleteIpMsg(ids, delImportant, false);
    }
    ...
    public static int getStatusResourceId(Context context, MessageItem msgItem) {
        ...
        MmsLog.d(TAG, "getStatusResourceId(): isMms = " + msgItem.isMms()
            ...
            + ", has read report = " + msgItem.mReadReport
            + ", has delivery report = " + (msgItem.mDeliveryStatus == MessageItem.DeliveryStatus.RECEIVED));
        if (msgItem.isMms()) {
