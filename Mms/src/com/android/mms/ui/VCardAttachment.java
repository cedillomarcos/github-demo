...
public class VCardAttachment {
    ...
    public String getVCardFileNameByContactsId(long[] contactsIds, boolean isAddingIsmsVCardFile) {
        MmsLog.d(TAG, "getVCardFileNameByContactsId(): isAddingIsmsVCardFile = " + isAddingIsmsVCardFile);
        ...
        if (!ids.equals("")) {
            ...
            fileName = attachVCard(ids, isAddingIsmsVCardFile);
        }
        return fileName;
    }
     ...
     * @param mContactIds
     */
    public String attachVCard(String mContactIds, boolean isAddingIsmsVCardFile) {
        if (mContactIds == null && mContactIds.equals("")) {
            return new String();
        }
        String fileName = isAddingIsmsVCardFile ? mVCardFileName : createVCardFileName();
        try {
            ...
            try {
                if (isAddingIsmsVCardFile) {
                    writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(
                        NmsCommonUtils.getCachePath(mContext) + fileName)));
                } else {
    ...
    private String getVCardFileNameForIpMessage(long[] contactIds) {
        ...
        if (contactIds.length == 1) {
            ...
            fileName = mContext.getResources().getString(R.string.ipmsg_vcard_file_name, contactIds.length);
        }
        return fileName + ".vcf";
    }
}
