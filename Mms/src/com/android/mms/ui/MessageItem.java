...
public class MessageItem {
    ...
    private static String ISMS_TAG = "Mms/isms/MessageItem";
    ...
    MessageItem(Context context, int boxId, int messageType, int simId, int errorType, int locked, int charset,
        ...
        if (!isOutgoingMessage()) {
            /// M: add for iSMS, change showing month-day to showing hour-minutes
            if (PduHeaders.MESSAGE_TYPE_NOTIFICATION_IND == mMessageType) {
                mTimestamp = context.getString(getTimestampStrId(), MessageUtils
                        .formatTimeStampString(context, timestamp));
            } else {
                mTimestamp = com.android.mms.isms.util.message.MessageUtils.getShortTimeString(
                    context, timestamp);
            }
        }
        ...
        if (mIsDrawTimeDivider) {
            if (PduHeaders.MESSAGE_TYPE_NOTIFICATION_IND == mMessageType) {
                mTimeDividerString = com.android.mms.isms.util.message.MessageUtils.getTimeDividerString(context, indDate * 1000L);
            } else {
                mTimeDividerString = com.android.mms.isms.util.message.MessageUtils.getTimeDividerString(context, timestamp);
            }
        }
        mIsDrawUnreadDivider = isDrawUnreadDivider;
        mUnreadCount = unreadCount;
        mIsDrawLoadAllButton = isDrawLoadAllMessagesButton;
        MmsLog.d(ISMS_TAG, "MessageItem(): show time divider ?= " + mIsDrawTimeDivider + ", mTimeDividerString = "
    ...
    MessageItem(Context context, String type, Cursor cursor, ColumnsMap columnsMap, Pattern highlight,
        ...
        if (!MessageUtils.getIpMessagePlugin(context).isActualPlugin()) {
            mIpMessageId = 0;
        }
        if (mIpMessageId > 0 && MmsConfig.getIpMessagServiceId(context) == IpMessageServiceId.ISMS_SERVICE) {
            mIpMessage = MessageUtils.getMessageManager(context).getIpMsgInfo(mMsgId);
            if (mIpMessage == null) {
                MmsLog.d(ISMS_TAG, "MessageItem.init(): ip message is null!");
            }
        }
        /// M: do not use this, use mLocked. It doesn't record right status for mms.
        ...
        mUnreadCount = unreadCount;
        mIsDrawLoadAllButton = isDrawLoadAllMessagesButton;
        MmsLog.d(ISMS_TAG, "MessageItem.init(): mIpMessageId = " + mIpMessageId + ", mImportant = " + mImportant
                ...
                if (date != 0) {
                    ...
                    if (isReceivedMessage()) {
                        if (mSimMsg) {
                            ...
                            mTimestamp = com.android.mms.isms.util.message.MessageUtils
                                    .getShortTimeString(context, date);
                        }
                        mSmsSentDate = cursor.getLong(columnsMap.mColumnSmsDateSent);
                    } else {
//                        mTimestamp = String.format(context.getString(R.string.sent_on),
//                                MessageUtils.formatTimeStampString(context, date));
                        mTimestamp = com.android.mms.isms.util.message.MessageUtils.getShortTimeString(context, date);
                    }
                } else {
                    mTimestamp = "";
                }
            ...
            if (mIsDrawTimeDivider) {
                mTimeDividerString = com.android.mms.isms.util.message.MessageUtils.getTimeDividerString(context, date);
            }
    ...
    public class PduLoadedMessageItemCallback implements ItemLoadedCallback {
        public void onItemLoaded(Object result, Throwable exception) {
            ...
            if (!isOutgoingMessage()) {
                ...
                if (PduHeaders.MESSAGE_TYPE_NOTIFICATION_IND == mMessageType) {
                    ...
                    mTimestamp = mContext.getString(getTimestampStrId(),
                        com.android.mms.isms.util.message.MessageUtils.getShortTimeString(mContext,
                            timestamp));
                }
            }
            ...
            if (mIsDrawTimeDivider) {
                if (PduHeaders.MESSAGE_TYPE_NOTIFICATION_IND == mMessageType) {
                    mTimeDividerString = com.android.mms.isms.util.message.MessageUtils
                            .getTimeDividerString(mContext, indDate * 1000L);
                } else {
                    mTimeDividerString = com.android.mms.isms.util.message.MessageUtils
                            .getTimeDividerString(mContext, timestamp);
                }
            }
