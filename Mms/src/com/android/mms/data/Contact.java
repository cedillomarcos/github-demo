...
//modify GPBYL-132 wangxiaobo 20130516 (start)
    public static Contact get(String number, boolean canBlock, long contactId, long dataId) {
        Log.i(TAG,">>>>>get contact by contactId="+contactId+" dataId="+dataId+" number="+number);
        return sContactCache.get(number, canBlock, contactId, dataId);
    }
//modify GPBYL-132 wangxiaobo 20130516 (end)
    ...
    public synchronized Drawable getAvatar(Context context, Drawable defaultValue, long threadId) {
        if (mAvatar == null) {
            if (mAvatarData != null) {
        ...
        if (mAvatar == null && MessageUtils.getIpMessagePlugin(context).isActualPlugin()) {
            if (!mIpMessageAvatarFetched) {
                if (threadId > 0 && MessageUtils.getContactManager(context).isIpMessageNumber(mNumber)) {
                    Bitmap bitmap = MessageUtils.getContactManager(context).getAvatarByThreadId(threadId);
                    if (null != bitmap) {
                        mIpMessageAvatar = new BitmapDrawable(bitmap);
                    }
                }
    ...
    //modify GPBYL-132 wangxiaobo 20130516 (start)
        public Contact get(String number, boolean canBlock, long contactId, long dataId) {
            return get(number, false, canBlock, contactId, dataId);
        }
    //modify GPBYL-132 wangxiaobo 20130516 (end)
...
//modify GPBYL-132 wangxiaobo 20130516 (end)
        private Contact get(String number, boolean isMe, boolean canBlock, long contactId, long dataId) {
            if (Log.isLoggable(LogTag.CONTACT, Log.DEBUG)) {
                logWithTrace("get(%s, %s, %s)", number, isMe, canBlock);
            }
            /// M:
            ...
            // in the contacts db.
            Contact contact = internalGet(number, isMe);
            if (contactId > 0 && dataId > 0) {
                contact.mPersonId = contactId;
                contact.mContactMethodId = dataId;
            }
            Runnable r = null;
...
//modify GPBYL-132 wangxiaobo 20130516 (end)
                ...
                if (contactChanged(c, entry)) {
                    ...
                    if (c.mThreadId > 0) {
                        ...
                        if (TextUtils.isEmpty(c.mName) || c.mNumber.equals(c.mName)) {
                            c.mName = MessageUtils.getContactManager(mContext).getNameByThreadId(c.mThreadId);
        ...
        private Contact getContactInfo(Contact c) {
            ...
            //modify GPBYL-132 wangxiaobo 20130516 (start)
				if (c.mPersonId > 0 && c.mContactMethodId > 0) {
					contact = getContactInfoForEmailAddressExt(c.mNumber, c.mPersonId, c.mContactMethodId);
					if (contact != null) {
						return contact;
					}
				}
	...
	//modify GPBYL-132 wangxiaobo 20130516 (start)
        /**
         * Queries the caller id info with the phone number and personId.
         * @return a Contact containing the caller id info corresponding to the number and personId.
         */
        private Contact getContactInfoForPhoneNumberExt(String number, long personId, long contactMethodId) {
            /// M: @{
            ...
            if (!TextUtils.isEmpty(normalizedNumber) && !TextUtils.isEmpty(matchNumber)) {
                ...
                //MmsLog.d(M_TAG, "getContactInfoForPhoneNumberExt(): exact query number=" + normalizedNumber);
                
                StringBuilder idSetBuilder = new StringBuilder();
                idSetBuilder.append(personId);

                StringBuilder idSetBuilder2 = new StringBuilder();
                idSetBuilder2.append(contactMethodId);
                
                Cursor cursor = null;
                if (idSetBuilder.length() > 0 && idSetBuilder2.length() > 0) {
                    cursor = mContext.getContentResolver().query(
                            PHONES_WITH_PRESENCE_URI, CALLER_ID_PROJECTION, Phone.CONTACT_ID + " IN (" + idSetBuilder.toString() + ")" + " AND " + CALLER_ID_SELECTION_EXACT_MATCH, args, null);
                }
                ...
                try {
                    if (cursor.moveToFirst()) {
                        /// M:
                        MmsLog.d(M_TAG,
                            "getContactInfoForPhoneNumberExt(): ready to fill contact with query result. number="
                                + normalizedNumber);
                        fillPhoneTypeContact(entry, cursor);
                    }
                } finally {
...
//                /// @}
        ...
        private void fillPhoneTypeContact(final Contact contact, final Cursor cursor) {
            ...
            if (MmsConfig.getIpMessagServiceId(mContext) == IpMessageServiceId.ISMS_SERVICE
                    && contact.getNumber().startsWith(MessageUtils.GROUP_START_STR)) {
                return;
            }
...
//modify GPBYL-132 wangxiaobo 20130516 (end)
        /**
         * Query the contact email table to get the name of an email address exact.
         */
        private Contact getContactInfoForEmailAddressExt(String email, long personId, long contactMethodId) {
            /// M:
            ...
            cursor = SqliteWrapper.query(mContext, mContext.getContentResolver(),
                    ...
                    EMAIL_PROJECTION,
                    Phone.CONTACT_ID + " IN (" + idSetBuilder.toString() + ")" + " AND " + EMAIL_SELECTION,
                    new String[] { email },
                    null);
            ...
            if (cursor != null) {
                ...
                MmsLog.d(M_TAG, "getContactInfoForEmailAddressExt(): cursor == null so we return null");
                return null;
            }
            return entry;
        }
//modify GPBYL-132 wangxiaobo 20130516 (end)
        ...
        /**
         * Query the contact e90mail table to get the name of an email address.
         */
    ...
    public static String getValidNumber(String numberOrEmail) {
        ...
        if (MmsConfig.getIpMessagServiceId(MmsApp.getApplication()) == IpMessageServiceId.ISMS_SERVICE &&
            numberOrEmail.startsWith(MessageUtils.GROUP_START_STR)) {
    ...
    public synchronized Drawable getGroupAvatar(Context context, long threadId) {
        MmsLog.d(TAG, "Contact.getGroupAvatar(): mAvatar is null ?= " + (mAvatar == null));
        if (mAvatar == null) {
            Bitmap bitmap = MessageUtils.getContactManager(context).getAvatarByThreadId(threadId);
            MmsLog.d(TAG, "Contact.getGroupAvatar(): bitmap is null ?= " + (bitmap == null));
            if (null != bitmap) {
