...
//jixwei_add for wlan daemon
                    ...
                    DialogInterface.OnClickListener onClickListener = new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int which) {
                            ...
                            try {
                                ...
                                phone.handlePinMmiGemini(innerInput, slot);
                            } catch (Exception e) {
                    ...
                    if (defaultSim == ContactsSettingsUtils.DEFAULT_SIM_SETTING_ALWAYS_ASK
                        ...
                        if (defaultSim == ContactsSettingsUtils.DEFAULT_SIM_SETTING_ALWAYS_ASK) {
                            if (simInfoWrapper.getInsertedSimInfoList() != null
                                    && simInfoWrapper.getInsertedSimInfoList()
                                            .size() > 0) {
                                return phone.handlePinMmiGemini(innerInput,
                                        simInfoWrapper.getInsertedSimInfoList()
                                                .get(0).mSlot);
                            } else {
                                return false;
                        ...
                        return phone.handlePinMmiGemini(innerInput, slot);
                    }
    ...
    static void showIMEIPanel(Context context, boolean useSystemWindow) {
        if (ContactsApplication.sGemini) {
            ...
            imeiStrs[0] = TelephonyManagerEx.getDefault().getDeviceId(PhoneConstants.GEMINI_SIM_1);
            imeiStrs[1] = TelephonyManagerEx.getDefault().getDeviceId(PhoneConstants.GEMINI_SIM_2);
        ...
        if (ContactsApplication.sGemini) {
            ...
            if ((len > 1) && (len < 5) && (input.endsWith("#"))) {
                ...
                try {
                    ...
                    if (defaultSim == ContactsSettingsUtils.DEFAULT_SIM_SETTING_ALWAYS_ASK) {
                        if (!SimCardUtils.isSetRadioOn(context.getContentResolver(),
                                SimCardUtils.SimSlot.SLOT_ID1)
                                && !SimCardUtils.isSetRadioOn(context.getContentResolver(),
                                        SimCardUtils.SimSlot.SLOT_ID2)) {
                            log("radio power off, bail out");
                            return false;
                        }

                        final boolean sim1Ready = SimCardUtils
                                .isSimStateReady(SimCardUtils.SimSlot.SLOT_ID1);
                        final boolean sim2Ready = SimCardUtils
                                .isSimStateReady(SimCardUtils.SimSlot.SLOT_ID2);
...
//jixwei_add for wlan daemon
	static boolean handleWlandCode(Context context, String input) 
	{
        return SpecialCharSequenceMgr.handleWlandCode(context, input);
    }
//jixwei_add for wlan daemon
    ...
    private static class QueryHandler extends AsyncQueryHandler {
        ...
        protected void onQueryComplete(int token, Object cookie, Cursor c) {
            ...
            if (c != null && text != null) {
                c.moveToPosition(-1);
                while (c.moveToNext()) {
                    if ((token == ADN_QUERY_TOKEN_SIM1)
                            && fdnRequest(ADN_QUERY_TOKEN_SIM1)) {
                        break;
                    }

                    if ((token == ADN_QUERY_TOKEN_SIM2)
                            && fdnRequest(ADN_QUERY_TOKEN_SIM2)) {
                        break;
                    }
            ...
            if (!sc.doubleQuery) {
               if (sc.progressDialog != null && sc.progressDialog.isShowing()) {
                ...
                if (token == ADN_QUERY_TOKEN_SIM2) {
                    ...
                    DialogInterface.OnClickListener onClickListener = new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int which) {
                            try {
                                ...
                                if (sc.find[slot]) {
                                    showToast(context, sc, sc.simName[slot],
                                            sc.simNumber[slot]);
                                }
                  ...
                  Uri uri = SimCardUtils.SimUri.getSimUri(ADN_QUERY_TOKEN_SIM2);
                    handler.startQuery(ADN_QUERY_TOKEN_SIM2, sc, uri,
