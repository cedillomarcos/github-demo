...
//    private static final String SIM_NUM_PATTERN = "[+]?[[0-9][*#]]+[[0-9][*#,]]*";
    ...
    public void onCreate(Bundle savedState) {
        ...
        try {
            if (FeatureOption.MTK_GEMINI_SUPPORT) {
                if (mITel != null && mITel.getIccCardTypeGemini(mSlotId).equals("USIM")) {
                    mSimType = "USIM";
                }
            } else {
                if (mITel.getIccCardType().equals("USIM")) {
                    mSimType = "USIM";
                }
            }
        } catch (Exception ex) {
        ...
        if (!TextUtils.isEmpty(sOtherPhone)) {
            ...
            Log.i(TAG, "*********** after split sOtherPhone number " + sAfterOtherPhone
                    + ",check valid:" + PhoneNumberUtils.extractCLIRPortion(sAfterOtherPhone));
    ...
    private class InsertSimContactThread extends Thread {
        ...
        private void checkPhoneStatus() {
            if (FeatureOption.MTK_GEMINI_SUPPORT) {
                try {
                    if (mITel != null && !mITel.isRadioOnGemini(mSlotId)) {
                        mAirPlaneModeOn = true;
                    }
                    if (mITel != null && mITel.isFDNEnabledGemini(mSlotId)) {
                        mFDNEnabled = true;
                    }
                ...
                try {
                    if (mITel != null && !mITel.isRadioOn()) {
                        mAirPlaneModeOn = true;
                    }
                    if (mITel != null && mITel.isFDNEnabled()) {
                        mFDNEnabled = true;
                    }
                    if (!(TelephonyManager.SIM_STATE_READY == TelephonyManager.getDefault()
                            .getSimState())) {
