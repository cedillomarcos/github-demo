...
//    private ContactPhotoLoader mPhotoLoader;
    ...
    protected void onListItemClick(ListView l, View v, int position, long id) {
        ...
        //add GWLSSW-42  pengshaopeng 20120702 (on) 
  		if (position == 0) 
  		{
  			Intent intent = new Intent();					
  			ComponentName cN = new ComponentName("com.android.phone", "com.android.phone.CallFeaturesSetting");
  			intent.setComponent(cN);
  			intent.putExtra("PACKAGE_CONTACTS_VOICEMAIL", "fun_call");
  			startActivity(intent);		
  			
  			return;
  		}
  	    //add GWLSSW-42  pengshaopeng 20120702 (off) 
    ...
    private boolean isSimReady(final int simId) {
        ...
        try {
            if (FeatureOption.MTK_GEMINI_SUPPORT) {
                ...
                boolean meLock = TelephonyManager.SIM_STATE_READY == TelephonyManagerEx.getDefault()
                        .getSimState(slotId);
                ...
                Log.d(TAG, "isSimReady(), ContactsUtils.isServiceRunning[slotId]=  "
                        + ContactsUtils.isServiceRunning[slotId]);

                if (mITel.isRadioOnGemini(slotId)) {
                    return !mITel.hasIccCardGemini(slotId)
                            || (mITel.isRadioOnGemini(slotId) && !mITel.isFDNEnabledGemini(slotId)
