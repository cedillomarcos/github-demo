...
public class DialerSearchAdapter extends CursorAdapter implements OnClickListener, View.OnTouchListener {
    ...
    public DialerSearchAdapter(Context context, Listener listener) {
        ...
        // 1. incoming 2. outgoing 3. missed 4. auto
    ...
    public void onResume() {
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            mVoiceMailNumber = TelephonyManagerEx.getDefault().getVoiceMailNumber(
                    PhoneConstants.GEMINI_SIM_1);
            mVoiceMailNumber2 = TelephonyManagerEx.getDefault().getVoiceMailNumber(
            ...
            if (FeatureOption.MTK_GEMINI_SUPPORT) {
                ...
                if (slot == PhoneConstants.GEMINI_SIM_1) {
                    if (mVoiceMailNumber != null
                            && PhoneNumberUtils.compare(mVoiceMailNumber, number)) {
                        return NUMBER_TYPE_VOICEMAIL;
                    }
                } else if (slot == PhoneConstants.GEMINI_SIM_2) {
                    if (mVoiceMailNumber2 != null
                            && PhoneNumberUtils.compare(mVoiceMailNumber2, number)) {
                        return NUMBER_TYPE_VOICEMAIL;
                    }
                }
            } else {
...
//    }
    ...
    public void bindContactView(View view, Context context, Cursor cursor) {
	    ...
	    //cuinana  GPBYY-60 modify   viewHolder.photo.assignContactUri(null);
