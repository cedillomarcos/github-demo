...
//        private final int mSimSlotId;
    ...
    class SimReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (Intent.ACTION_AIRPLANE_MODE_CHANGED.equals(action)) {
                ...
                if (AccountWithDataSetEx.isSimAccount(mAccountType)) {
                    clearMembersList();
                }
    ...
    void processAirplaneModeChanged(Intent intent) {
        ...
        if (isAirplaneModeOn && AccountWithDataSetEx.isSimAccount(mAccountType)) {
            clearMembersList();
        }
        if (isAirplaneModeOn) {
            scrubActivity();
    ...
    private void showDialog() {
        ...
        sProgressDialog.setTargetFragment(GroupEditorFragment.this, 0);
        sProgressDialog.show(GroupEditorFragment.this.getFragmentManager(), "wait");
    ...
    private void dismissDialog() {
        ...
        if (sProgressDialog != null && sProgressDialog.getDialog() != null
            ...
            Log.d(TAG, "sProgressDialog.dismiss()");
    ...
    private static class MemberTask extends AsyncTask<ArrayList, Void, ArrayList> {
        ...
        private static MemberTask sInstance = null;
        ...
        public static MemberTask getTask() {
            if (sInstance != null) {
                Log.d(TAG, "MemberTask getTask() sInstance.cancel(true)");
                sInstance.cancel(true);
                sInstance = null;
            }
            sInstance = new MemberTask();
            return sInstance;
        }
        ...
        protected ArrayList<Member> doInBackground(ArrayList... params) {
            ...
            if (contactIdss.size() > 0) {
                selection += contactIdss.get(0);
            }
            for (int i = 1; i < contactIdss.size(); i++) {
                selection += "," + contactIdss.get(i);
            }
            selection += ")";
