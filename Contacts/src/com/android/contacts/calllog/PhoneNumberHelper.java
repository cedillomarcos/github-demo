...
public class PhoneNumberHelper {
        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            ...
            if (slot == PhoneConstants.GEMINI_SIM_1) {
                if (sVoiceMailNumber != null
                        && PhoneNumberUtils.compare(sVoiceMailNumber.toString(), 
                                                    number.toString())) {
                    return true;
                }

            } else if (slot == PhoneConstants.GEMINI_SIM_2) {
                if (sVoiceMailNumber2 != null
                        && PhoneNumberUtils
                                .compare(sVoiceMailNumber2.toString(), number.toString())) {
                    return true;
                }
            }
        } else {
            if (sVoiceMailNumber != null
                    && PhoneNumberUtils.compare(sVoiceMailNumber.toString(), number.toString())) {
                return true;
            }
        }
        return false;
    ...
    private static String sVoiceMailNumber2;

    public static void getVoiceMailNumber() {
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            sVoiceMailNumber = TelephonyManagerEx.getDefault().getVoiceMailNumber(
                    PhoneConstants.GEMINI_SIM_1);
            sVoiceMailNumber2 = TelephonyManagerEx.getDefault().getVoiceMailNumber(
                    PhoneConstants.GEMINI_SIM_2);
        } else {
            sVoiceMailNumber = TelephonyManager.getDefault().getVoiceMailNumber();
        }
    }
