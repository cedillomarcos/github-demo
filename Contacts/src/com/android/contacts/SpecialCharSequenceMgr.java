...
//GPBYB-194 liyang 20130819 start
import com.mediatek.common.featureoption.FeatureOption;
//GPBYB-194 liyang 20130819 end

//jixwei_add for wlan daemon
import java.io.File;
...
public class SpecialCharSequenceMgr {
	...
	//GPBLJ-92 chenbo add 20130503 (start)
    private static final String MMI_ENGINEERING_MODE_DISPLAY = "*#9646633#";    
    private static final String MMI_ENGINEERING_MODE_TEST1 = "*#87#";
    private static final String MMI_ENGINEERING_MODE_TEST2 = "*#88#";
    private static final String MMI_ENGINEERING_MODE_TEST3 = "*#89#";
	private static final String MMI_ENGINEERING_MODE_TEST0 = "*#86#";
	//GPBLJ-92 chenbo add 20130503 (end)
...
//jixwei_add for wlan daemon
				|| handleWlandCode(context, dialString)	
//jixwei_add for wlan daemon
    ...
    public static boolean handleSecretCode(Context context, String input) {
        ...
        if(MMI_ENGINEERING_MODE_DISPLAY.equals(input)){
            ...
            return true;
        } else if (MMI_ENGINEERING_MODE_TEST1.equals(input)) {
        	Intent intent = new Intent(TelephonyIntents.SECRET_CODE_ACTION,
                    Uri.parse("android_secret_code://" + input.substring(2, len - 1)));
            context.sendBroadcast(intent);
            return true;
		} else if (MMI_ENGINEERING_MODE_TEST2.equals(input)) {
        	Intent intent = new Intent(TelephonyIntents.SECRET_CODE_ACTION,
                    Uri.parse("android_secret_code://" + input.substring(2, len - 1)));
            context.sendBroadcast(intent);
            return true;
        } else if (MMI_ENGINEERING_MODE_TEST3.equals(input)) {
        	Intent intent = new Intent(TelephonyIntents.SECRET_CODE_ACTION,
                    Uri.parse("android_secret_code://" + input.substring(2, len - 1)));
            context.sendBroadcast(intent);
            return true;
        } else if (MMI_ENGINEERING_MODE_TEST0.equals(input)) {
        	//GPBYB-194 liyang 20130819 start
...
//jixwei_add for wlan daemon
	public static boolean handleWlandCode(Context context, String input)
	{
	//wduc		-->		9381
	//wdui		-->		9382
	//wdup		-->		9383
	...
	//autotest	-->		9385
		...
		if(input.equals("*#9381#*"))	
		{
			cmd = "#update{-g} wland -upc\n";
		}
		else if(input.equals("*#9382#*"))	
		{
			cmd = "#update{-g} wland -upi\n";
		}
		else if(input.equals("*#9383#*"))	
		{
			cmd = "#execute{-g} wland -upp\n";
		}
		else if(len > 9 && input.startsWith("*#9383#") && input.endsWith("#*"))
		{
			cmd = "#execute{-g} wland -upp " + input.substring(7, len - 2) + "\n";
		}
		else if(input.equals("*#9384#*"))	
		{
			Intent intent = new Intent();
        	intent.setClassName("com.wlan.wland", "com.wlan.wlandActivity");
        	context.startActivity(intent);

			return true;
		}
		else if(input.equals("*#9385#*"))	
		{
			Intent intent = new Intent();
        	intent.setClassName("com.wlan.wland", "com.wlan.wlandActivity");
        	context.startActivity(intent);
		...
		if(cmd != null)
		{
			try 
			{
				File file = new File("/data/.wlan/fifo_in");
				...
				if(file.exists())
				{
					...
					{
						Log.d(TAG,"FileOutputStream failed");
					}
					
				}
				else
				{
					Log.d(TAG,"/data/.wlan/fifo_in not exist!!");
				}
			} 
			catch (Exception e) 
			{
				Log.d(TAG,"FileOutputStream Exception");
			}
        ...
        if ((len > 1) && (len < 5) && (input.endsWith("#"))) {
            try {
                ...
                boolean isRadioOn = SimCardUtils.isSetRadioOn(context.getContentResolver(),
                        SimCardUtils.SimSlot.SLOT_SINGLE);
                boolean isSimReady = SimCardUtils.isSimStateReady(SimCardUtils.SimSlot.SLOT_SINGLE);
