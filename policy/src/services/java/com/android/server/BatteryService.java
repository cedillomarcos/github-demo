...
public final class BatteryService extends Binder {
    ...
    private void shutdownIfNoPowerLocked() {
        ...
        if (mBatteryLevel == 0 && (!isPoweredLocked(BatteryManager.BATTERY_PLUGGED_ANY) || (FeatureOption.MTK_BATLOWV_FORCE_SHUTDOWN == true))) {        
    ...
    private void processValuesLocked() {
        ...
        // add by chenweiwei start
        //adjustLevel();
        // add by chenweiwei end
    ...
    private int mPrevPlugType = -1;
    private long mPrevTime = -1;
    private boolean mIsStatusChange = false;
    // add by chenweiwei end

    private void adjustLevel() {
        if (mPrevLevel == -1) {
            mPrevLevel = mBatteryLevel;
        }
        if (mPrevTime == -1) {
            mPrevTime = System.currentTimeMillis();
        }
        if (mPrevPlugType == -1) {
            mPrevPlugType = mPlugType;
        }

        if (mPrevPlugType != mPlugType) {
            mPrevPlugType = mPlugType;
            mPrevTime = System.currentTimeMillis();
        ...
        if (!mIsStatusChange) {
            if (Math.abs(System.currentTimeMillis() - mPrevTime) > 120000) {
            int dValue = Math.abs(mBatteryLevel - mPrevLevel);
            
            Slog.d(TAG, "Sending ACTION_BATTERY_CHANGED. original mBatteryLevel = " + mBatteryLevel);

            if (dValue != 0) {
                if (mBatteryLevel > mPrevLevel) {
                    mBatteryLevel = mPrevLevel + ((dValue > 1) ? 1:dValue);
                } else {
                    mBatteryLevel = mPrevLevel - ((dValue > 1) ? 1:dValue);
                }
                mPrevTime = System.currentTimeMillis();
            }
            Slog.d(TAG, "Sending ACTION_BATTERY_CHANGED.  dValue:" + dValue
                   + ", mBatteryLevel = " + mBatteryLevel
                   + ", mPrevLevel = " + mPrevLevel);
            mPrevLevel = mBatteryLevel;
	    }else{
	     mBatteryLevel = mPrevLevel;
	    }
        }

        if (mIsStatusChange) {
            if (Math.abs(System.currentTimeMillis() - mPrevTime) > 30000) {
mBatteryLevel = mPrevLevel;
                mPrevTime = System.currentTimeMillis();
                mIsStatusChange = false;
            } else {
                mBatteryLevel = mPrevLevel;
            }
        }
        ...
        Slog.d(TAG, "Sending ACTION_BATTERY_CHANGED.  mPrevTime:" + mPrevTime
                    + ", mPrevPlugType = " + mPrevPlugType
                    + ", mIsStatusChange = " + mIsStatusChange
                    + ", mPrevLevel = " + mPrevLevel);
        Slog.d(TAG, "Sending ACTION_BATTERY_CHANGED.  mCurrTime:" + System.currentTimeMillis()
                    + ", mPlugType = " + mPlugType
                    + ", mBatteryLevel = " + mBatteryLevel);
Slog.d("renxinquan","send out battery ="+mBatteryLevel);
    }
