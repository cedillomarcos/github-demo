...
public final class PowerManagerService extends IPowerManager.Stub
    ...
    private void updateWakeLockSummaryLocked(int dirty) {
        if ((dirty & (DIRTY_WAKE_LOCKS | DIRTY_WAKEFULNESS)) != 0) {
            ...
            final int numWakeLocks = mWakeLocks.size();
            //Add by mtk94141 for WAKE LOCK Exception handling mechanism
            if(FeatureOption.MTK_WAKE_LOCK_ERROR_HANDLING) {
                int curAppPid = mActivityManagerService.getForegroundAppPid();
                for (int i = 0; i < numWakeLocks; i++) {
                    final WakeLock wakeLock = mWakeLocks.get(i);
                    Slog.d(TAG, "wakeLock.pid= " + wakeLock.mOwnerPid + ",tag= " + wakeLock.mTag);
                    switch (wakeLock.mFlags & PowerManager.WAKE_LOCK_LEVEL_MASK) {
                        case PowerManager.PARTIAL_WAKE_LOCK:
                            mWakeLockSummary |= WAKE_LOCK_CPU;
                            break;
                        case PowerManager.FULL_WAKE_LOCK:
                            if (curAppPid < 0 ||(!KeyguardViewMediator.isKeyguardShowingAndNotHidden()
                                    && (wakeLock.mOwnerPid == curAppPid))) {
                                ...
                                Slog.d(TAG, "ignore FULL_WAKE_LOCK,wakelock.pid= " + wakeLock.mOwnerPid);
                            }
                            break;
                        case PowerManager.SCREEN_BRIGHT_WAKE_LOCK:
                            if (curAppPid < 0 ||(!KeyguardViewMediator.isKeyguardShowingAndNotHidden()
                                && (wakeLock.mTag == "WindowManager" || (wakeLock.mOwnerPid == curAppPid)))) {
                                if (mWakefulness != WAKEFULNESS_ASLEEP) {
                                ...
                                Slog.d(TAG, "ignore SCREEN_BRIGHT_WAKE_LOCK,wakelock.pid= " + wakeLock.mOwnerPid);    
                            }
                            break;
                        case PowerManager.SCREEN_DIM_WAKE_LOCK:
                            if (curAppPid < 0 ||(!KeyguardViewMediator.isKeyguardShowingAndNotHidden()
                                ...
                                Slog.d(TAG, "ignore SCREEN_DIM_WAKE_LOCK,wakelock.pid= " + wakeLock.mOwnerPid);
                            }
                            break;
                        case PowerManager.PROXIMITY_SCREEN_OFF_WAKE_LOCK:
                            if (mWakefulness != WAKEFULNESS_ASLEEP) {
    ...
    private void updateUserActivitySummaryLocked(long now, int dirty) {
		...
		//add GPBYL-943 yukai 20130903 begin
		if(mScreenOffTimeoutSetting == -1){
			mLastWakeTime = now;
			mLastUserActivityTime = now ;
			mLastUserActivityButtonTime = now ;
		}
		//add GPBYL-943 yukai 20130903 end
