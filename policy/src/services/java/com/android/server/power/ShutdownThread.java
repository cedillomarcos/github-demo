...
public final class ShutdownThread extends Thread {
	...
	boolean rebuildDialog;
        if(FeatureOption.RLK_REBOOT_SUPPORT){
		isRebooting = mReboot && !mRebootSafeMode;
		Log.d(TAG, "Notifying thread to start radio shutdown");
		bConfirmForAnimation = confirm;
		longPressBehavior = context.getResources().getInteger(
		        com.android.internal.R.integer.config_longPressOnPowerBehavior);       
		resourceId = isRebooting ? com.android.internal.R.string.reboot_confirm
		    : (mRebootSafeMode
...
//add zhaojiangwei test
        if(FeatureOption.RLK_REBOOT_SUPPORT){
            if (sConfirmDialog == null || rebuildDialog) {
                Log.d(TAG, "PowerOff dialog doesn't exist. Create it first");
                sConfirmDialog = new AlertDialog.Builder(context)
                    .setTitle(isRebooting ? com.android.internal.R.string.reboot_title
...
//GPBYL-648 display andoird deafult shutanimation
if ((FeatureOption.RLK_GP810H_F1_ZX_SUPPORT) || (FeatureOption.RLK_GP810H_J1_ZX_SUPPORT) || (FeatureOption.RLK_GP818H_A1_SN_SUPPORT) || (FeatureOption.RLK_GP811H_A1_SUPPORT)){
 	mShutOffAnimation = true;
}
        synchronized (mEnableAnimatingSync) {
...
//                sInstance.mPowerManager.setBacklightBrightness(PowerManager.BRIGHTNESS_DIM);
                ...
                if (mShutOffAnimation) {
                    ...
                    //add zhaojiangwei test
		    if(FeatureOption.RLK_REBOOT_SUPPORT){
		            boolean isRebooting = mReboot && !mRebootSafeMode;
		            if (isRebooting) {
