...
class GlobalActions implements DialogInterface.OnDismissListener, DialogInterface.OnClickListener  {
    ...
    //add zhaojiangwei test
    private ToggleAction mPowerModeOn;
    private ToggleAction.State mPowerModeState;
...
//add zhaojiangwei test
	public void createProgressDialog() {
		...
		mProgressDialog.setMessage(mContext.getResources().getString(R.string.is_switch_warning));
		mProgressDialog.setCancelable(false);
		mProgressDialog.setCanceledOnTouchOutside(false);
		mProgressDialog.getWindow().setType(WindowManager.LayoutParams.TYPE_SYSTEM_ALERT);
		//mProgressDialog.getWindow().addFlags(WindowManager.LayoutParams.FLAG_HOMEKEY_DISPATCHED);
		}
	}
	...
	if (mProgressDialog != null) {
		if (show) {
			...
			if (mProgressDialog.isShowing()) {
				...
				Settings.Global.SWITCH_POWER_MODE,
				0);
			}
		}
	}
	...
	public void removeAllRecentTasksList(){
		...
		for(int i = 0; i < recentTasks.size(); i++){
			...
			//Log.i("zjw", "zjw launcher="+islauncherApp);
		...
		//if(!isTopActivity("com.android.launcher"))
		{
...
if(FeatureOption.RLK_POWER_MODE){
	...
	mCheckServer = new CheckServer(mContext);
	boolean inPowerMode;
	inPowerMode = Settings.Global.getInt(mContext.getContentResolver(), Settings.Global.POWER_MODE_ON, 0) != 0;
	mPowerModeState = inPowerMode ? ToggleAction.State.On : ToggleAction.State.Off;

     mPowerModeOn = new ToggleAction(
                R.drawable.ic_lock_powermode_on,
                R.drawable.ic_lock_powermode_off,
                R.string.menu_power_mode,
                R.string.start_narmal,
                R.string.end_normal) {
		...
		if((inPowerSwitchPowerMode && (mProgressDialog!=null && mProgressDialog.isShowing())) || inMenuSwitchPowerMode){
		...
		mPowerModeState = on ? ToggleAction.State.On : ToggleAction.State.Off;
		...
		if(mPowerModeState == ToggleAction.State.On){
		mCheckServer.start_setting();
		}else{		
		mCheckServer.restore_setting();
		}
		/*GESYSW-878 zhaojiangwei 0730 +++ */
		...
		mPowerModeState = buttonOn ? ToggleAction.State.On : ToggleAction.State.Off; 
            }
	...
	}else if(mCallState != TelephonyManager.CALL_STATE_OFFHOOK){
            inPowerMode = Settings.Global.getInt(mContext.getContentResolver(), Settings.Global.POWER_MODE_ON, 0) != 0;
	mPowerModeState = inPowerMode == false ? ToggleAction.State.Off : ToggleAction.State.On; 
        mPowerModeOn.updateState(mPowerModeState);
	}
}
//add zhaojiangwei test
	...
	//add zhaojiangwei test
	if(FeatureOption.RLK_REBOOT_SUPPORT){
		mItems.add(
		    new SinglePressAction(
		            com.android.internal.R.drawable.ic_lockscreen_restart,
		            R.string.reboot_title) {
