...
import com.android.internal.policy.impl.keyguard.CenterKeyListener;
...
public class MediatekGlowPadView extends View implements OnTouchListener {
    ...
    //zhuwei add
    private static final int MSG_PLAY_JIANTOU_ANIM = 1;
    private static final int MSG_PLAY_BREATH_ANIM = 2;
    private static final int MSG_DELAY_CHECK_STATE = 3;
    private static final int MSG_UNLOCK = 4;
    private static final int PLAY = 1;
    private static final int DELAY_CHECK_TIME = 500;
    private static final int DELAY_JIANTOU_ANIM = 1100;
    private static final int DELAY_REPEAT_JIANTOU_ANIM = 1500;
    private static final int DELAY_UNLOCK = 500;
    
    private static final int ADD_MINIMUM_HEIGHT = 0;
    private static final int ADD_MINIMUM_WITTH = 0;
    
    private static final int ADD_DIMENSION_WIDTH = 0;
    private static final int ADD_DIMENSION_HEIGHT = 0;
    
    
   /* private static final int DEFAULT_DESIRED_WIDTH = 858;
    private static final int DEFAULT_DESIRED_HEIGHT = 858;
    
    private static final int DEFAULT_ACTUAL_WIDTH = 858;
    private static final int DEFAULT_ACTUAL_HEIGHT = 858;*/
    
    private static final int ORIENTATION_PROT_CENTER_X_OFFSET = -57;
    private static final int ORIENTATION_PROT_CENTER_Y_OFFSET = -70;
    
    private static final int ORIENTATION_LAND_CENTER_X_OFFSET = 0;
    private static final int ORIENTATION_LAND_CENTER_Y_OFFSET = -80;
    
    
    private static final float RING_SCALE_FACTOR = 0.82f;
    
    private static final int OUTER_RADIUS = 290;
    ...
    private TargetDrawable mMmsDrawable;
    private TargetDrawable mPhoneDrawable;
    private MmsUnReadObserver obMms;
    private MissCallUnReadObserver obMissCall;
    
    private GestureDetector mCenterKeyGestureDetector;
    
    private CenterKeyGestureListener mCenterKeyGestureListener;
    
    private boolean mShowingBatteryInfo = true;
    private boolean mCharging = false;
    private boolean mBatteryCharged = false;
    
    private boolean showChargingCenterKey = false;
    private boolean showFlashLightCenterKey = false;
    private boolean isFlashLightOn = false;
    
    
    private boolean handleCenterKey = false;
    
    
    
    private class CenterKeyGestureListener extends android.view.GestureDetector.SimpleOnGestureListener {
    	
    	@Override
    	public boolean onDoubleTap(MotionEvent e) {
    		Log.i("zhuwei_click", "onDoubleTap");
    		mClickHandler.removeCallbacks(checkForPressState);
    		mClickHandler.sendEmptyMessage(MSG_HANDLER_DOUBLE_CLICK);
    		return super.onDoubleTap(e);
    	}
    	
    	@Override
    	public void onLongPress(MotionEvent e) {
    		Log.i("zhuwei_click", "onLongPress");
    		mClickHandler.sendEmptyMessage(MSG_HANDLER_LONG_PRESSED);
    		mClickHandler.removeCallbacks(checkForPressState);
    	//	mClickHandler.post(checkForPressState);
    		super.onLongPress(e);
    	}
    	...
    	public boolean onSingleTapUp(MotionEvent e) {
    		Log.i("zhuwei_click", "onSingleTapUp");
    		mClickHandler.removeCallbacks(checkForPressState);
    ...
    private int getCenterKeyState() {
    	int state = CenterKeyListener.LOCKSCREEN_LEVEL_NORMAL;
    	if (mCenterKeyListener != null) {
    		state = mCenterKeyListener.getCenterKeyState();
    	}
    	if (showFlashLightCenterKey) {
    		if (isFlashLightOn) {
    			state = CenterKeyListener.LOCKSCREEN_LEVEL_FLASH_LIGHT_ON;
    		} else {
    			state = CenterKeyListener.LOCKSCREEN_LEVEL_FLASH_LIGHT_OFF;
    		}
    	} else if (showChargingCenterKey) {
    		state = CenterKeyListener.LOCKSCREEN_LEVEL_CHARGING;
    	}
    	Log.i("zhuwei_anim", "getCenterKeyState from listener:state-->"+state);
    	return state;
    }
    
    public void changeCenterKeyState() {
    	mHandleDrawable.setLevel(getCenterKeyState());
    }
    
    private boolean isInCenterRegion(float x, float y) {
    	boolean isInRegion = false;
    	
    	float offset = (float) Math.sqrt(dist2(mHandleDrawable.getWidth()/2, 
    			mHandleDrawable.getWidth()/2))/2;
    	
    	
    	float minX = mWaveCenterX - offset;
    	float minY = mWaveCenterY - offset;
    	
    	float maxX = mWaveCenterX + offset;
    	float maxY = mWaveCenterY + offset;
    ...
    private Handler mHandler = new Handler() {
    	public void handleMessage(Message msg) {
    		int msgId = msg.what;
    		int play = msg.arg1;
    		if (msgId == MSG_PLAY_JIANTOU_ANIM) {
    			if (mAnimListener != null) {
    				if (play == PLAY) {
    					mHandler.removeMessages(MSG_PLAY_BREATH_ANIM);
    					mHandler.removeMessages(MSG_PLAY_JIANTOU_ANIM);
                                        mAnimListener.playJiantouAnim(false);
    					mAnimListener.playBackGroundAnim(false);
    					mAnimListener.playJiantouAnim(true);
    					Message jiantouMsg = new Message();
    					jiantouMsg.arg1 = PLAY;
    					jiantouMsg.what = MSG_PLAY_JIANTOU_ANIM;
    					mHandler.sendMessageDelayed(jiantouMsg, DELAY_REPEAT_JIANTOU_ANIM);
    				} else {
    					mAnimListener.playJiantouAnim(false);
    				}
    			}
    		} else if (msgId == MSG_PLAY_BREATH_ANIM) {
    			if (mAnimListener != null) {
    				if (play == PLAY) {
    					mHandler.removeMessages(MSG_PLAY_BREATH_ANIM);
    					mHandler.removeMessages(MSG_PLAY_JIANTOU_ANIM);
    					mAnimListener.playJiantouAnim(false);
    					mAnimListener.playBackGroundAnim(false);
    					mAnimListener.playBackGroundAnim(true);
    					Message breathMsg = new Message();
    					breathMsg.arg1 = PLAY;
    					breathMsg.what = MSG_PLAY_JIANTOU_ANIM;
    					mHandler.sendMessageDelayed(breathMsg, DELAY_JIANTOU_ANIM);
    				} else {
    					mAnimListener.playBackGroundAnim(false);
    				}
    			}
    		} else if (msgId == MSG_DELAY_CHECK_STATE) {
    			if (mAnimListener != null) {
    				if (mShowingBatteryInfo && mCharging && !mBatteryCharged) {
    	            	if (mAnimListener != null) {
    	            		mAnimListener.playChargingAnim(true);
    	            		showChargingCenterKey = true;
    	            	}
    	            } else {
    	            	if (mAnimListener != null) {
    	            		mAnimListener.playChargingAnim(false);
    	            		showChargingCenterKey = false;
    	            	}
    	            }
    			}
    			changeCenterKeyState();
    		} else if (msgId == MSG_UNLOCK) {
    			if (mCenterKeyListener != null) {
    ...
    public void setCenterKeyListener(CenterKeyListener mCenterKeyListener) {
    ...
    public MediatekGlowPadView(Context context, AttributeSet attrs) {
        ...
        //zhuwei modify
        mHandleDrawable = new TargetDrawable(context, handle != null ? handle.resourceId : 0);
        ...
        if (FeatureOption.FEATURE_GP811_LOCK_SCREEN) {
        	 if (mCenterKeyListener != null) {
             	...
             	Log.e("zhuwei", "4mCenterKeyListener == null!!");
             	mHandler.sendEmptyMessageDelayed(MSG_DELAY_CHECK_STATE, DELAY_CHECK_TIME);
             }
        }
        //end
        ...
        //zhuwei add
        if (FeatureOption.FEATURE_GP811_LOCK_SCREEN) {
             mCenterKeyGestureDetector = new GestureDetector(getContext(), new CenterKeyGestureListener());
        }
    ...
    public void resumeAnimations() {
        ...
        if (FeatureOption.FEATURE_GP811_LOCK_SCREEN) {
        	if (mAnimListener != null) {
        		...
        		msg.what = MSG_PLAY_JIANTOU_ANIM;
        		msg.arg1 = PLAY;
        		mHandler.sendMessageDelayed(msg, DELAY_JIANTOU_ANIM);
        	}
        }
    }
    ...
    protected int getSuggestedMinimumWidth() {
        ...
        return (int) (Math.max(mOuterRing.getWidth(), 2 * mOuterRadius) + mMaxTargetWidth + ADD_MINIMUM_WITTH);
    }
    ...
    protected int getSuggestedMinimumHeight() {
        ...
        // target drawable on either edge
        return (int) (Math.max(mOuterRing.getHeight(), 2 * mOuterRadius) + mMaxTargetHeight + ADD_MINIMUM_HEIGHT);
    }
    ...
    private void switchToState(int state, float x, float y) {
        switch (state) {
            ...
            case STATE_FIRST_TOUCH:
                ...
                if (FeatureOption.FEATURE_GP811_LOCK_SCREEN) {
                	 if (mCenterKeyListener != null) {
                     	...
                     	Log.e("zhuwei", "1mCenterKeyListener == null!!");
                     }
                     changeCenterKeyState();
                } else {
            ...
            case STATE_TRACKING:
                ...
                if (FeatureOption.FEATURE_GP811_LOCK_SCREEN) {
                    mHandleDrawable.setAlpha(1.0f);
                    mHandleDrawable.setLevel(CenterKeyListener.LOCKSCREEN_LEVEL_HANDLER_MOVE);
                    mHandler.removeMessages(MSG_PLAY_JIANTOU_ANIM);
                    mHandler.removeMessages(MSG_PLAY_BREATH_ANIM);
                    if (mAnimListener != null) {
                	    mAnimListener.playBackGroundAnim(false);
    ...
    //zhuwei add
    private void resetHandleDrawable() {
        if (mCenterKeyListener != null) {
        	mHandleDrawable.setAlpha(1.0f);
        	mShowingBatteryInfo = true;
        	if (mCharging  && !mBatteryCharged) {
        	...
        	mHandler.removeMessages(MSG_PLAY_JIANTOU_ANIM);
        	Message msg = new Message();
        	msg.what = MSG_PLAY_BREATH_ANIM;
        	msg.arg1 = PLAY;
        	mHandler.sendMessageDelayed(msg, 500);
        } else {
        	Log.e("zhuwei", "3mCenterKeyListener == null!!");
        }
        if (true) {
    ...
    private ArrayList<TargetDrawable> loadDrawableArray(int resourceId) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            //zhuwei add
            if (value.resourceId == R.drawable.gp811_ic_lockscreen_sms) {
            	mMmsDrawable = target;
            } else if (value.resourceId ==  R.drawable.gp811_ic_lockscreen_phone) {
            	mPhoneDrawable = target;
            }
            //end
    ...
    private static final int UNLOCK_AND_OPEN_FLASH_LIGHT_TIME = 5000;
    
    private static final int MSG_HANDLER_LONG_PRESSED = 3;
    private static final int MSG_HANDLER_DOUBLE_CLICK = 4;
    private static final int MSG_HANDLER_OPEN_FLASH_LIGHT = 5;
    
    private Handler mClickHandler = new Handler() {
    	public void handleMessage(Message msg) {
    		int msgID = msg.what;
    		if (msgID == MSG_HANDLER_DOUBLE_CLICK) {
    			handleDoubleClick();
    		} else if (msgID == MSG_HANDLER_LONG_PRESSED) {
    			handleLongPressed();
    		} else if (msgID == MSG_HANDLER_OPEN_FLASH_LIGHT) {
    			Log.i("zhuwei_click", "handler MSG_HANDLER_OPEN_FLASH_LIGHT");
    			handleOpenFlashApk();
    		}
    	}
    };
    ...
    private void handleDoubleClick() {
    	if (mCenterKeyListener != null) {
    		mCenterKeyListener.handleDoubleClick();
    	}
    }
    ...
    private void handleLongPressed() {
    	if (mCameraDevice == null)
		{
			try
			{
				...
				{
					resetHandleDrawable();
				//	Toast.makeText(this, "Camera open failed!", 2000).show();
					return;
				}
			}
			catch(Exception e)
			{
				resetHandleDrawable();
			//	Toast.makeText(this, "Camera open Exception!", 2000).show();
				return;
			}
		}
		if (mParameters == null)
		{
			...
			{	
				resetHandleDrawable();
				//Toast.makeText(this, "Camera param get failed!", 2000).show();
				return;
			}
		}
		if (isFlashLightOn) {
			...
			showFlashLightCenterKey = false;
			changeCenterKeyState();
		} else {
			mParameters.setFlashMode(Camera.Parameters.FLASH_MODE_TORCH);
			mCameraDevice.setParameters(mParameters);
			isFlashLightOn = true;
			showFlashLightCenterKey = true;
			changeCenterKeyState();
		}
	}
    
    private void handleOpenFlashApk() {
		/*
		...
		mIntent.setClassName("rgt.activity.torch",
				"rgt.activity.torch.TorchActivity");
		getContext().startActivity(mIntent);
		
    	mHandler.sendEmptyMessageDelayed(MSG_UNLOCK, DELAY_UNLOCK);
		*/
    }
    
    
    private Runnable checkForPressState = new Runnable() {
		
		@Override
		public void run() {
			try {
				Thread.sleep(UNLOCK_AND_OPEN_FLASH_LIGHT_TIME);
			} catch (InterruptedException e) {
				Log.i("zhuwei_click", "InterruptedUNLOCK_AND_OPEN_FLASH_LIGHT ");
			}
			mClickHandler.sendEmptyMessage(MSG_HANDLER_OPEN_FLASH_LIGHT);
		}
	};
    ...
    private void updateGlowPosition(float x, float y) {
        ...
        //zhuwei modify
        if (FeatureOption.FEATURE_GP811_LOCK_SCREEN) {
        	float x1 = mOuterRing.getX() + dx;
        	float y1 = mOuterRing.getY() + dy;
        	float z1 = (float) Math.sqrt(dist2(x1, y1));
        	float measure = mOuterRadius;
        	if (y1 < 0) {
        		measure = measure - mHandleDrawable.getWidth()/2;
        	}
        	if (y1 > 0) {
        		measure = measure - mHandleDrawable.getWidth()/2;
        	}
        	...
        	mHandleDrawable.setX(x1);
        	mHandleDrawable.setY(y1);
        } else {
    ...
    //zhuwei add
    private double getTargetRad(float mOuterRadius, float tatgetRadius) {
    	float x = (float) Math.sqrt(dist2(mOuterRadius, mOuterRadius));
    	float y = (float) Math.sqrt(dist2(tatgetRadius, tatgetRadius));
    	return y / x;
    }
    //end
    ...
    private void handleMove(MotionEvent event) {
        ...
        if (actionIndex == -1) {
            return;  // no data for this pointerI
        }
        ...
        for (int k = 0; k < historySize + 1; k++) {
            ...
            if (mDragging) {
                ...
                for (int i = 0; i < ntargets; i++) {
                    ...
                    //zhuwei add
                    if (FeatureOption.FEATURE_GP811_LOCK_SCREEN) {
                    	 double offset = getTargetRad(mOuterRadius, target.getHeight()/2);
                    	 targetMinRad = mFirstItemOffset + (i - offset) * 2 * Math.PI / ntargets;
                         targetMaxRad = mFirstItemOffset + (i + offset) * 2 * Math.PI / ntargets;
                    }
                    //end
                    if (target.isEnabled()) {
        ...
        if (activeTarget != -1) {
            ...
            //zhuwei add
            if (FeatureOption.FEATURE_GP811_LOCK_SCREEN) {
            	 if (!handleCenterKey && isInCenterRegion(x,y)) {
                 	mCenterKeyGestureDetector.onTouchEvent(event);
                 } else {
                 	handleCenterKey = true;
                 	mClickHandler.removeCallbacks(checkForPressState);
                 	updateGlowPosition(x, y);
                 }
            } else {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        //zhuwei add
        if (FeatureOption.FEATURE_GP811_LOCK_SCREEN) {
        	mRingScaleFactor = RING_SCALE_FACTOR;
        }
        //end
        ...
        //zhuwei add
        if (FeatureOption.FEATURE_GP811_LOCK_SCREEN) {
        	setMeasuredDimension(computedWidth + ADD_DIMENSION_WIDTH, computedHeight + ADD_DIMENSION_HEIGHT);
        } else {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (FeatureOption.FEATURE_GP811_LOCK_SCREEN) {
        	if (getDisplayOrientation() == Configuration.ORIENTATION_PORTRAIT) {
        		 newWaveCenterX = newWaveCenterX + ORIENTATION_PROT_CENTER_X_OFFSET;
                 newWaveCenterY = newWaveCenterY + ORIENTATION_PROT_CENTER_Y_OFFSET;
        	} else {
        		newWaveCenterX = newWaveCenterX + ORIENTATION_LAND_CENTER_X_OFFSET;
                newWaveCenterY = newWaveCenterY + ORIENTATION_LAND_CENTER_Y_OFFSET;
        	}
        }
    ...
    private void updateTargetPosition(int i, float centerX, float centerY, float angle) {
        ...
        if (i >= 0) {
            ...
            //zhuwei add
            int id = targetIcon.getResourceId();
            int X = (int) (placementRadiusX * (float) Math.cos(angle));
            int Y = (int) (placementRadiusY * (float) Math.sin(angle));
            int height = targetIcon.getHeight();
            int width = targetIcon.getWidth();
            int offset = 20;
            switch (id) {
			case R.drawable.gp811_ic_lockscreen_sms: {
				X = X + width/2 + offset;
				break;
			}
			case R.drawable.gp811_ic_lockscreen_phone: {
				X  = X - width/2 - offset;
				break;
			}
			case R.drawable.gp811_ic_lockscreen_key: {
				Y = Y + height/2 + offset;
				break;
			}
			default:
    ...
    private BroadcastReceiver mClearUnReadTipRceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            mQueryBaseTime = currentTime;
        }
    };
    
    private void registerNewEventObserver(TargetDrawable mTargetDrawable) {
        int resourceId = mTargetDrawable.getResourceId();
        switch (resourceId) {
            case R.drawable.gp811_ic_lockscreen_sms:
                obMms = new MmsUnReadObserver(new Handler(), mTargetDrawable, mQueryBaseTime);
                getContext().getContentResolver().registerContentObserver(
                		MmsUnReadObserver.MMS_URI, true, obMms);
                obMms.refreshUnReadNumber();
                break;
            case R.drawable.gp811_ic_lockscreen_phone:
                obMissCall = new MissCallUnReadObserver(new Handler(), mTargetDrawable, mQueryBaseTime);
                getContext().getContentResolver().registerContentObserver(
                		MissCallUnReadObserver.MISS_CALL_URI, true, obMissCall);
    ...
    public void setUnReadVisibility(int visiblity) {
    	if (mMmsDrawable != null) {
    		mMmsDrawable.setUnReadVisibility(visiblity);
    	}
    	if (mPhoneDrawable != null) {
    		mPhoneDrawable.setUnReadVisibility(visiblity);
    	}
    }
    ...
    public void onResume() {
    	 //zhuwei add
        if (mTargetDrawables == null || mTargetDrawables.size() <= 0
        		|| mMmsDrawable == null || mPhoneDrawable == null) {
            return;
        } else {
        	int count = mTargetDrawables.size();
        	for (int i = 0; i < count; i++) {
        		registerNewEventObserver(mTargetDrawables.get(i));
        	}
        }
        ...
        } else if (mCharging && !mBatteryCharged) {
        	if (mAnimListener != null) {
    			mAnimListener.playChargingAnim(true);	
    			showChargingCenterKey = true;
    		}
    ...
    public void onPause() {
        if (obMms != null && obMissCall != null) {
            getContext().getContentResolver().unregisterContentObserver(obMms);
            getContext().getContentResolver().unregisterContentObserver(obMissCall);
        }
        if (mAnimListener != null) {
        	mAnimListener.playBackGroundAnim(false);
        	mAnimListener.playChargingAnim(false);
        ...
        //end
        Log.i("renxinquan","onPause onPause onPause");
		if(mCameraDevice!=null){
    ...
    private void hideOtherNewEventViews() {
        ...
        if (mUnReadEventView == null) {
        	Log.i("zhuwei", "MediatekGlowPadView mUnReadEventView == null!");
        	return;
        }
