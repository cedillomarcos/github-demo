...
import com.android.internal.policy.impl.keyguard.CoatLockScreen;
import com.android.internal.policy.impl.keyguard.CoatLockScreen.OnHitListener;
//renxinquan add end
...
public class KeyguardHostView extends KeyguardViewBase {
    ...
    //zhuwei add
    private int mCenterKeyState = CenterKeyListener.LOCKSCREEN_LEVEL_NORMAL;
    private MediatekGlowPadView mMediatekGlowPadView;
    private View mFmMusicView;
    private MusicHelper mMusicHelper;
    private FMHelper mFMHelper;
    private View mMusicView;
    private View mFmView;
    //end
    
    //liyang 20131021 add start
    private TextView mMusicFmInfo;
    ...
    private int mUserId;
	  //renxinquan add start
	  private CoatLockScreen mDragLayer;
	  //renxinquan add end
	  
	//liyang add start
	private final static int MSG_UPDATE_CENTERKEY_INFO = 1;
	//liyang add end
    ...
    protected void onFinishInflate() {
        ...
        if (FeatureOption.FEATURE_GP811_LOCK_SCREEN) {
        	mMediatekGlowPadView = (MediatekGlowPadView)findViewById(R.id.glow_pad_view);
            if (mMediatekGlowPadView != null) {
            	Log.i("zhuwei", "mMediatekGlowPadView != null setCenterKeyListener");
            	mCenterKeyState = CenterKeyListener.LOCKSCREEN_LEVEL_NORMAL;
            	mMediatekGlowPadView.setCenterKeyListener(mCenterKeyListener);
            }
            mMusicView = findViewById(R.id.music_main_view);
            mFmMusicView = findViewById(R.id.fm_music_frame);
            mFmView = findViewById(R.id.fm_main_view);
            mMusicView.setVisibility(View.INVISIBLE);
            mFmView.setVisibility(View.INVISIBLE);
            
            mMusicHelper = new MusicHelper(mContext, mMusicView, mCenterKeyListener);
       	    mFMHelper =  new FMHelper(mContext, mFmView, mCenterKeyListener);
       	    SecurityMode mSecurityMode = mSecurityModel.getSecurityMode();
            if (mSecurityMode != SecurityMode.None) {
            	mFMHelper.stop();
            	mFMHelper.destroy();
            	mMusicHelper.stop();
            	mMusicHelper.destroy();
            	mMusicView.setVisibility(View.GONE);
            	mFmView.setVisibility(View.GONE);
            } 
        }
        //end
    ...
    //zhwei add
    private void notifyChangeCenterKeyState() {
    	if (mMediatekGlowPadView != null) {
    		Log.i("zhuwei","host call mediatekglowpadview changecenterkey state!");
    		mMediatekGlowPadView.changeCenterKeyState();
    	}
    }
    ...
    private CenterKeyListener mCenterKeyListener = new CenterKeyListener() {
    	
    	private int state = STATE_ERROR;
    	private static final int MUSIC_FM_ANIM_DURATION = 500;
    	
    	public int getCenterKeyState() {
        	return mCenterKeyState;
        }
		...
		public void changeState(int state) {
			if (mCenterKeyState != state) {
				mCenterKeyState = state;
				Log.i("zhuwei", "CenterKeyListener ->changeState :"+state);
			}
			notifyChangeCenterKeyState();
		}

		@Override
		public int handleDoubleClick() {
			if (mFmView == null || mMusicView == null 
					|| mFMHelper ==null || mMusicHelper == null) {
				Log.i("zhuwei_anim", "handleDoubleClick error!");
				return STATE_ERROR;
			}
			boolean fmViewVisible = mFmView.getVisibility() == View.VISIBLE;
			boolean musicViewVisible = mMusicView.getVisibility() == View.VISIBLE;
			if (fmViewVisible && !musicViewVisible) {
				playAnim(mFmView, null, mFMHelper, null,false);
				state = STATE_FM;
				mHandler.sendEmptyMessage(MSG_UPDATE_CENTERKEY_INFO);
				return state;
			} else if (!fmViewVisible && musicViewVisible) {
				playAnim(mMusicView, null, mMusicHelper, null, false);
				state = STATE_MUSIC;
				mHandler.sendEmptyMessage(MSG_UPDATE_CENTERKEY_INFO);
				return state;
			} else if (!fmViewVisible && !musicViewVisible) {
				playAnim(mMusicView, null, mMusicHelper, null, true);
				//liyang 20131021 add start
				updateMusicFmInfo();
				//liyang 20131021 add end
				return state;
		...
		@Override
		public boolean handleLongPressed() {
			return false;
		}

		@Override
		public boolean gotoFM() {
			if (mFmView == null || mMusicView == null 
					|| mFMHelper ==null || mMusicHelper == null) {
				return false;
			}
			...
			boolean musicViewVisible = mMusicView.getVisibility() == View.VISIBLE;
			if (!fmViewVisible && musicViewVisible) {
				playAnim(mMusicView, mFmView, mMusicHelper, mFMHelper,false);
				return true;
			} else {
				Log.i("zhuwei_change", "gotoFM error!!!!fmViewVisible->"+fmViewVisible
						+",musicViewVisible->"+musicViewVisible);
				return false;
			}
		}

		@Override
		public boolean gotoMusic() {
			if (mFmView == null || mMusicView == null 
					|| mFMHelper ==null || mMusicHelper == null) {
				return false;
			}
			...
			boolean musicViewVisible = mMusicView.getVisibility() == View.VISIBLE;
			if (fmViewVisible && !musicViewVisible) {
				playAnim(mFmView, mMusicView, mFMHelper, mMusicHelper, false);
				return true;
			} else {
				Log.i("zhuwei_change", "gotoMusic error!!!!fmViewVisible->"+fmViewVisible
						+",musicViewVisible->"+musicViewVisible);
				return false;
			}
		...
		private void playAnim(final View from, final View to, 
				final Object fromHelp, final Object toHelp,final boolean visible) {
			if (from == null || fromHelp == null) {
				Log.e("zhuwei_anim", "v == null can not playAnimAndSetVisible");
				return;
			}
			
			float fromAlpha = 0.0f;
			float toAlpha = 0.0f;
			float fromPositionX = 0.0f;
			float toPositionX = 0.0f;
			float fromPositionY = 0.0f;
			float toPositionY = 0.0f;
			
			if (visible) {
				fromAlpha = 0.0f;
				toAlpha = 1.0f;
				//fromPositionX = from.getRight();
                fromPositionX = from.getLeft();
				toPositionX = from.getLeft();
				//fromPositionY = from.getTop();
				fromPositionY = from.getBottom();
				toPositionY = from.getTop();
			} else {
				fromAlpha = 1.0f;
				toAlpha = 0.0f;
				fromPositionX = from.getLeft();
		    ...
		    AnimationSet mAnimSet = getAnimSet(from, fromAlpha, toAlpha, 
		    ...
		    mAnimSet.setAnimationListener(new AnimationListener() {
				...
				public void onAnimationEnd(Animation animation) {
					from.setVisibility(visible?View.VISIBLE:View.INVISIBLE);
					if (fromHelp instanceof MusicHelper) {
						if (visible) {
							((MusicHelper) fromHelp).startMusicService();
							((MusicHelper) fromHelp).startUpdate();
						} else {
							((MusicHelper) fromHelp).stop();
						}
					} else {
						if (visible) {
							((FMHelper)fromHelp).startFMService();
							((FMHelper)fromHelp).startUpdate();
						} else {
							((FMHelper)fromHelp).stop();
						}
					}
					if (to != null && toHelp != null) {
						AnimationSet mAnimSet = getAnimSet(to,0.0f,1.0f,
								to.getRight(),to.getRight(),
								to.getBottom(),to.getTop());
						mAnimSet.setAnimationListener(new AnimationListener() {
							...
							public void onAnimationEnd(Animation animation) {
								...
								if (toHelp instanceof MusicHelper) {
									((MusicHelper) toHelp).startMusicService();
									((MusicHelper) toHelp).startUpdate();
								} else {
									((FMHelper)toHelp).startFMService();
									((FMHelper)toHelp).startUpdate();
								}
								notifyChangeCenterKeyState();
							}
						});
						to.startAnimation(mAnimSet);
					}
					notifyChangeCenterKeyState();
				}
			});
		    from.startAnimation(mAnimSet);
		}
		
		 private AnimationSet getAnimSet(View v,
		    		float fromAlpha, float toAlpha,
		    		float fromPositionX, float toPositionX,
		    		float fromPositionY,float toPositionY) {
		    	  Animation mAlphaAnim = new AlphaAnimation(fromAlpha,toAlpha);
				  Animation translatAnim = new TranslateAnimation(
						  fromPositionX,toPositionX,
						  fromPositionY, toPositionY);
				    AnimationSet mAnimSet = new AnimationSet(true);
				    mAnimSet.addAnimation(mAlphaAnim);
				    mAnimSet.addAnimation(translatAnim);
				    mAnimSet.setDuration(MUSIC_FM_ANIM_DURATION);
				    return mAnimSet;
		    }
    ...
    private KeyguardSecurityView getSecurityView(SecurityMode securityMode) {
        ...
        if (view == null && layoutId != 0) {
            ...
            if (view instanceof KeyguardSelectorView && bu.getState()) {
								if (mDragLayer == null) {
									mDragLayer = (CoatLockScreen) LayoutInflater.from(mContext).inflate(R.layout.keyguard_drag_view, null);
					        mDragLayer.setHitListener(new OnHitListener() {
    ...
    public void onScreenTurnedOn() {
        ...
        requestFocus();
         ...
         //zhuwei add
         if (FeatureOption.FEATURE_GP811_LOCK_SCREEN) {
        	 if (mMusicView != null && mFmMusicView != null && mFmView != null) {
            	 if (mMusicHelper != null && mFMHelper != null) {
            		 boolean isMusicOn = mMusicHelper.isPlaying();
                	 boolean isFMOn = mFMHelper.isPlaying();
                	 Log.i("zhuwei_fm_music", "onFinishInfalte,isMusicOn = "+isMusicOn
                			 +",isFMoN-->"+isFMOn);
                	 if (isMusicOn) {
                	 ...
                	 if (isFMOn) {
                		 mFmView.setVisibility(View.VISIBLE);
                	 } else {
                		 mFmView.setVisibility(View.GONE);
                		 mFMHelper.stop();
                	 }
    ...
    public void onScreenTurnedOff() {
        ...
        //zhuwei add
        if (FeatureOption.FEATURE_GP811_LOCK_SCREEN) {
        	 if (mFMHelper != null && mMusicHelper != null) {
                 mFMHelper.stop();
                 mFMHelper.destroy();
                 mMusicHelper.stop();
                 mMusicHelper.destroy();  
            ...
            if (mMediatekGlowPadView != null) {
            	mCenterKeyState = CenterKeyListener.LOCKSCREEN_LEVEL_NORMAL;
           	    mMediatekGlowPadView.onPause();
            }
        }
       //end
        
        //liyang add start
        mHandler.removeMessages(MSG_UPDATE_CENTERKEY_INFO);
        mHandler.removeCallbacks(updateCenterKeyInfo);
        //liyang add end
    }
    ...
    private Handler mHandler = new Handler(){
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case MSG_UPDATE_CENTERKEY_INFO:
                    mHandler.post(updateCenterKeyInfo);
                    break;
    ...
    private final Runnable updateCenterKeyInfo = new Runnable() {
        ...
        public void run() {
            ...
            mHandler.postDelayed(updateCenterKeyInfo, 500);
        }
    };
    //liyang add end
    
  //liyang 20131021 add start
    public void updateMusicFmInfo(){
        CharSequence string = null;
        boolean fmViewVisible = mFmView.getVisibility() == View.VISIBLE;
        boolean musicViewVisible = mMusicView.getVisibility() == View.VISIBLE;
        boolean isMusicOn = mMusicHelper.isPlaying();
        boolean isFMOn = mFMHelper.isPlaying();
        mMusicFmInfo = (TextView)findViewById(R.id.music_fm_info);
        
        Log.d("liyang", "fmViewVisible: " + fmViewVisible + ", musicViewVisible: " + musicViewVisible + ", isMusicOn: " + isMusicOn + ", isFMOn: " + isFMOn);
        if (!fmViewVisible && !musicViewVisible) {
            if (isMusicOn) {
                string = mMusicHelper.getMusicName();
            } else if (isFMOn){
                string = mFMHelper.getFMChanel();
            }else {
                string = null;
            }
            
            mMusicFmInfo.setText(string);
            mMusicFmInfo.setVisibility(View.VISIBLE);
        }else {
            mMusicFmInfo.setVisibility(View.GONE);
        }
    }
    //liyang 20131021 add end
    ...
    private int getLayoutIdFor(SecurityMode securityMode) {
        switch (securityMode) {
            //zhuwei modify
             case None: {
            	 if (FeatureOption.FEATURE_GP811_LOCK_SCREEN) {
            		 return R.layout.gp811_keyguard_selector_view; 
            	 } else {
    ...
    private void addDefaultStatusWidget(int index) {
        ...
        //zhuwei modify
        if (FeatureOption.FEATURE_GP811_LOCK_SCREEN) {
        	mStatusWidget = inflater.inflate(R.layout.gp811_keyguard_status_view, null, true);
        } else {
