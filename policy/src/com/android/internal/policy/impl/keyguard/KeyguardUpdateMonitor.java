...
public class KeyguardUpdateMonitor {
    ...
    private final Handler mHandler = new Handler() {
		...
		//liyang MTK add msg.arg1
                    handleTimeUpdate(msg.arg1);
                    break;
    ...
    private final BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
		...
		int isTimeZoneChanged;
		if(Intent.ACTION_TIMEZONE_CHANGED.equals(action)){
			isTimeZoneChanged = 1;
		}else{
			isTimeZoneChanged = 0;
		}
		final Message msg = mHandler.obtainMessage(MSG_TIME_UPDATE, isTimeZoneChanged, 0);
     ...
     */
    //liyang MTK add int isTimeZoneChanged
    private void handleTimeUpdate(int isTimeZoneChanged) {
        if (DEBUG) Log.d(TAG, "handleTimeUpdate isTimeZoneChanged=" + isTimeZoneChanged);
        for (int i = 0; i < mCallbacks.size(); i++) {
            KeyguardUpdateMonitorCallback cb = mCallbacks.get(i).get();
            if (cb != null) {
                cb.onTimeChanged(isTimeZoneChanged);
            }
        }
    }
