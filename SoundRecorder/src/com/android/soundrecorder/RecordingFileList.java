    ...
    public ArrayList<HashMap<String, Object>> queryData() {
        ...
        Cursor recordingFileCursor = getContentResolver().query(
                ...
                MediaStore.Audio.Media.IS_MUSIC + " =0 and " + MediaStore.Audio.Media.DATA
                        + " LIKE '" + "%" + "/" + Recorder.RECORD_FOLDER + "%" + "'", null,MediaStore.Audio.Media.DATE_ADDED + " DESC");//cuinana GPBYL-53
        try {
            if ((null == recordingFileCursor) || recordingFileCursor.getCount() == 0) {
        ...
        if (mCurrentAdapterMode == EDIT) {
            if (null != ((EditViewAdapter) mRecordingFileListView.getAdapter())) {
                ...
                if (!checkedList.isEmpty()) {
                    LogUtils.i(TAG, "<onPause> save checkedList");
                    mCheckedList = checkedList;
                    LogUtils.i(TAG, "<onPause> mCheckedList.size() = " + mCheckedList.size());
                }
            }
        } else {
            mCheckedList = null;
    ...
    public class FileTask extends AsyncTask<Void, Object, Boolean> {
        ...
        protected Boolean doInBackground(Void... params) {
            ...
            for (int i = 0; i < listSize; i++) {
                ...
                if (!SoundRecorderUtils.deleteFileFromMediaDB(getApplicationContext(), list
