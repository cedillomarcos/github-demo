...
import com.android.music.ext.Extensions;
import com.android.music.ext.IMusicTrackBrowser;

import com.android.music.MusicUtils.ServiceToken;

import java.util.Arrays;
//GPBYB-319 liyang 20130911 add start
import android.widget.Toast;
import com.mediatek.common.featureoption.FeatureOption;
//GPBYB-319 liyang 20130911 add end
        ...
        implements View.OnCreateContextMenuListener, MusicUtils.Defs, ServiceConnection, DialogInterface.OnClickListener
{
    ...
    /// M: plugin for Operator feature.
    private IMusicTrackBrowser mMusicTrackBrowserPlugin = null;
    /// M: enalbe clear playlsit menu in all trackbrower except RecentlyAdd and Poadcast.
    private boolean mEnableClearPlaylistMenu = false;
    ...
    public void onCreate(Bundle icicle)
    {
        ...
        // move to MusicBrowser; add get plungin for operator feature. @{
        if (!mWithtabs) {
            mToken = MusicUtils.bindToService(this, this);
            mMusicTrackBrowserPlugin = Extensions.createPluginObject(getApplication());
            if (mMusicTrackBrowserPlugin != null) {
                MusicLogUtils.d(TAG, "Get operator succeed");
                mEnableClearPlaylistMenu = mMusicTrackBrowserPlugin.enableClearPlaylistMenu();
            }
        }
        /// @}
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        if (mPlaylist != null) {
            ...
            /// M: Add clear playlist and add file to playlist option menu to playlist 
            // except "Recently added" and "Podcasts".
            boolean enable = !mPlaylist.equals("recentlyadded") && !mPlaylist.equals("podcasts");
            if ((enable && mEnableClearPlaylistMenu) || 
                    (!mEnableClearPlaylistMenu && 
                    mPlaylist.equals("nowplaying"))) {
                menu.add(0, CLEAR_PLAYLIST, 0, R.string.clear_playlist).setIcon(R.drawable.ic_menu_clear_playlist);
            }
            /// M: add file to playlist from filemanager @{
            if (mMusicTrackBrowserPlugin != null && enable) {
                mMusicTrackBrowserPlugin.createAddSongMenu(menu, ADD_SONG);
            }
            /// @}
        }
        /// M: Add effect panel
	...
	//GPBYB-319 liyang 20130911 add end
        MusicUtils.addSearchView(this, menu, mQueryTextListener);
        return true;
    }
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        if (mEnableClearPlaylistMenu && mPlaylist != null && menu.findItem(CLEAR_PLAYLIST) != null) {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        /// M: use plugin app to handle the Add Song Menu.
        if (mMusicTrackBrowserPlugin != null && 
                mMusicTrackBrowserPlugin.handleAddSongMenu(item, this)) {
            return true;
        }
        /// @}
        Intent intent;
            ...
            case SHARE_ITEM:
				try {
					if (mService.isPlaying()){
						ComponentName comp = new ComponentName("com.suning.mediaserver", "com.suning.mediaserver.activity.DlnaMediaServerCheck");
						Intent intent1 = new Intent();
						intent1.setComponent(comp);
						intent1.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
						intent1.putExtra("filePath", mService.getPath());
						intent1.putExtra("IsLocal", true);
						intent1.putExtra("MimeType", mService.getMIMEType());
						MusicLogUtils.d("liyang", "---->filePath: " +  mService.getPath() + ", MimeType: " +  mService.getMIMEType());
						startActivity(intent1);
					}else{
						Toast.makeText(getApplicationContext(), getString(R.string.select_one), Toast.LENGTH_LONG).show();
					}
				}catch (RemoteException ex){
					finish();
				}
    ...
    private void addSongToPlaylist(Uri uri) {
        ...
        MusicLogUtils.d(TAG, "addSongToPlaylist: where=" + where);
        ...
        if (c != null) {
            if (c.moveToFirst() && c.getCount() > 0) {
                ...
                MusicLogUtils.d(TAG, "addSongToPlaylist: track id=" + trackId);
                long[] list = new long[] { trackId };
                MusicLogUtils.d(TAG, "addSongToPlaylist: mPlaylist=" + mPlaylist);
                if (mPlaylist.equals("nowplaying")) {
                    MusicUtils.addToCurrentPlaylist(this, list);
                } else {
        ...
        if (showError && mMusicTrackBrowserPlugin != null) {
            String toastShow = mMusicTrackBrowserPlugin.getNoneAudioString();
            if (toastShow != null) {
        ...
        try {
            ...
            switch (rightsStatus) {
                ...
                case DrmStore.RightsStatus.RIGHTS_INVALID:
                    mDrmClient.showLicenseAcquisitionDialog(this, 
                        getUri(cursor), null);
                    break;
