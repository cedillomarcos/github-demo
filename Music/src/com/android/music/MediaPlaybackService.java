...
public class MediaPlaybackService extends Service {
    ...
    private static final int REMOVE_TRACK = 9;
...
// add GPBLJ-171 liyang start
    public static final String CMDUPDATE = "updatelist";
...
// add GPBLJ-171 liyang start
    private boolean isMusicUpdate = false;
// add GPBLJ-171 liyang end
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                     ...
                     switch (msg.arg1) {
                        ...
                        case AudioManager.AUDIOFOCUS_LOSS_TRANSIENT:
                            ...
                            if(isPlaying()) {
                                ...
                                //GPBYY-80 shuchenlin begin
                                Editor ed = mPreferences.edit();
                                ed.putBoolean("playing_before", true);
                                ed.commit();
                                //GPBYY-80 shuchenlin end
                            }
                            pause();
                            break;
                ...
                case CHANGE_SETTING_MODE:
                    ...
                    break;
                /// @}
                /// M: remove track in mainthread.It will be improved in furture @{
                case REMOVE_TRACK:
                    removeTrack(msg.arg1);
                    break;
                /// @}  
                default:
                    break;
    ...
    private BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
        ...
        // add GPBLJ-171 liyang start
            isMusicUpdate = intent.getBooleanExtra("ismusicupdate", false);
            MusicLogUtils.v(TAG, "isMusicUpdate=" + isMusicUpdate);
        // add GPBLJ-171 liyang end	    
            /// @}
            ...
            /// @}
                ...
                abortBroadcast();
                if (mPreferences.getBoolean("playing_before", false)) {
                    play();
                }
                //GPBYY-80 shuchenlin end
    ...
    // add GPBLJ-171 liyang start
    private BroadcastReceiver mScanSdReceiver = new BroadcastReceiver() {
        private int count;

        @Override 
        public void onReceive(Context context, Intent intent) { 
            String action = intent.getAction(); 
            MusicLogUtils.v(TAG, "onReceive isMusicUpdate=" + isMusicUpdate);
            if(isMusicUpdate) {
                if(Intent.ACTION_MEDIA_SCANNER_STARTED.equals(action)){ 

                    Cursor c1 = (Cursor)context.getContentResolver().query(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, 
                    ...
                    Toast.makeText(context, R.string.scan_card, Toast.LENGTH_LONG).show();
                }else if(Intent.ACTION_MEDIA_SCANNER_FINISHED.equals(action)){ 
                    Cursor c2 = (Cursor)context.getContentResolver().query(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, 
                            new String[]{MediaStore.Audio.Media.TITLE, 
                            MediaStore.Audio.Media.DURATION, 
                            MediaStore.Audio.Media.ARTIST, 
                            MediaStore.Audio.Media._ID, 
                    ...
                    count = c2.getCount();
                    Toast.makeText(context, String.format(getString(R.string.of_all), count), Toast.LENGTH_LONG).show();
                    isMusicUpdate = false;
                }    
            }
        } 
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent != null && !isEventFromMonkey()) {
            ...
            if (CMDNEXT.equals(cmd) || NEXT_ACTION.equals(action)) {
			    ...
			    //GPBYY-80 shuchenlin begin
                Editor ed = mPreferences.edit();
                ed.putBoolean("playing_before",false);
                ed.commit();
                //GPBYY-80 shuchenlin end
                pause();
                mPausedByTransientLossOfFocus = false;
...
//            ed.apply();
		...
		//GPBYB-138 liyang 20130814 start
		mQueueIsSaveable = true;
		//GPBYB-138 liyang 20130814 end
    ...
    public void enqueue(long [] list, int action) {
        synchronized(this) {
            if (action == NEXT && mPlayPos + 1 < mPlayListLen) {
                ...
                if (mPlayer.isInitialized() && mPlayPos == (mPlayListLen-1) ) {
                    mPlayer.setNextDataSource(null);
                }
                /// @}
    ...
    public void play() {
        synchronized(this) {
           final long max_Progress_Length = 1000;
            ...
            if (mPlayer.isInitialized() && mIsPlayerReady) {
                ...
                if (mRepeatMode != REPEAT_CURRENT && (mIsPlaylistCompleted || (duration == 0) ||
                            (duration > 0 && mPlayer.position() >= duration - duration % max_Progress_Length))) {
                    gotoNext(true);
    ...
    static class ServiceStub extends IMediaPlaybackService.Stub {
        ...
        public void removeTrackInMainThread(long id) {
            mService.get().removeTrackInMainThread(id);
        }
    }
    ...
    private void reloadQueueAfterScan() {
        mCardId = MusicUtils.getSDCardId(mStorageManager);
        MusicLogUtils.d(TAG,"reloadQueueAfterScan: mCardId = " + mCardId + ", isReloadSuccess = " + mIsReloadSuccess);
        if (mCardId != -1 && !mIsReloadSuccess
    ...
    /**
     * M:remove Track In MainThread,it will be imporved in furture
     * @param id
     */
    
    public void removeTrackInMainThread(long id){
        mMediaplayerHandler.sendMessage(mMediaplayerHandler.obtainMessage(REMOVE_TRACK, (int)id, 0));
    } 
}
