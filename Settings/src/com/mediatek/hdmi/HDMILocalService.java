...
public class HDMILocalService extends Service {
    ...
    public static int[] sCECCmd = null;
    ...
    public void setCECAddr(char laNum, char[] la, char pa, char svc) {
        Log.e(TAG, ">>HDMILocalService.setCECAddr");
        mHdmiNative.setCECAddr(laNum, la, pa, svc);
    }

    public void setCECCmd(char initAddr, char destAddr, char opCode, char[] operand, int size, char enqueueOk) {
    ...
    public void getCECCmd() {
        Log.i(TAG, ">>HDMILocalService.getCECCmd");
        sCECCmd = mHdmiNative.getCECCmd();

        if (sCECCmd != null) {            
            for (int i = 0; i < 22; i++) {
                Log.i(TAG, ">>HDMILocalService CEC command [" + i + "] = " + sCECCmd[i]);
            }
            //cecCmd.blocks.operand[14], Empty=0x33, UnEmpty=0x55
            if (sCECCmd[21] == 0x55) {
                //Clear previous CEC command
                //TODO: remember to store previous CEC commands
                sCECCmd = null;
                getCECCmd();
                Log.i(TAG, ">>HDMILocalService Need to get more CEC Commands!");
            }            
        } else {
            Log.i(TAG, ">>HDMILocalService.getCECCmd null");
        }
    }
...
//GPBYL-739(start)
       ...
       //GPBYL-739(end)
        ...
        Settings.System.putString(getContentResolver(), KEY_HDMI_VIDEO_RESOLUTION, videoResolution);
        setVideoResolution(Integer.parseInt(videoResolution));
    }
