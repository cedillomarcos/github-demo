...
public class BandwidthUsage extends Fragment {
    ...
    public void onResume() {
        ...
        try {
            boolean enable = mNetworkService.getInterfaceThrottleEnabled();
            Xlog.d(TAG,"onResume,getInterfaceRxThrottle=" + enable);
            mEnableThrottling.setChecked(enable);
            mChart.setLimitState(enable);
        } catch (RemoteException e) {
            Xlog.d(TAG, " RemoteException happens when getInterfaceThrottle");
        }
        ...
        public void onClick(View v) {
            try {
                if (mEnableThrottling.isChecked()) {
                    ...
                    mNetworkService.setInterfaceThrottleEnabled(false);
                    mChart.setLimitState(false);
                    setLimitData(false);
                } else {
                    mEnableThrottling.setChecked(true);
                    mNetworkService.setInterfaceThrottleEnabled(true);
                ...
                Xlog.d(TAG, " RemoteException happens when setInterfaceThrottle:" + e);
            }
        }
    };
