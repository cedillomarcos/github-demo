...
public class LbsReceiver extends BroadcastReceiver {
    ...
    private String mCurOperatorCodeTwo;
    ...
    private void handleOmaCpSetting(Context context, Intent intent) { //MTK_CS_IGNORE_THIS_LINE
        ...
        if (simId == PhoneConstants.GEMINI_SIM_1) {
            ...
            profileCode = mCurOperatorCodeTwo;
        }
    ...
    private void initSIMStatus(boolean isGemini) {
        ...
        if (isGemini) {
            sim1Status = telMgrEx.getSimState(PhoneConstants.GEMINI_SIM_1);
            sim2Status = telMgrEx.getSimState(PhoneConstants.GEMINI_SIM_2);

            if (TelephonyManager.SIM_STATE_READY == sim1Status && TelephonyManager.SIM_STATE_READY == sim2Status) {
                mCurSIMStatus = DUAL_SIM;
                mCurOperatorCodeOne = telMgrEx.getSimOperator(PhoneConstants.GEMINI_SIM_1);
                mCurOperatorCodeTwo = telMgrEx.getSimOperator(PhoneConstants.GEMINI_SIM_2);
            } else if (TelephonyManager.SIM_STATE_READY == sim1Status && TelephonyManager.SIM_STATE_READY != sim2Status) {
        ...
        log("mCurSIMStatus is: " + mCurSIMStatus);
        log("sim1 card status is: " + sim1Status);
        log("sim2 card status is: " + sim2Status);
        log("sim1 operator code is: " + mCurOperatorCodeOne);
        log("sim2 operator code is: " + mCurOperatorCodeTwo);
    }
