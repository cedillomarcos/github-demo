...
//modify GPBYL-123 20130522_maoyutao start
import java.util.ArrayList;
//modify GPBYL-123 20130522_maoyutao end
...
//modify GPBYL-123 20130522_maoyutao start
        Log.d(TAG, "currentEngine = " + currentEngine + ", currentLocale = " + currentLocale);
        if (currentLocale == null || "".equals(currentLocale.toString().trim())) {
        	Log.d(TAG, "mVoiceDataDetails = " + mVoiceDataDetails);
        	if (mVoiceDataDetails != null) {
        		synchronized(mVoiceDataDetails) {
                    ArrayList<String> available = mVoiceDataDetails.getStringArrayListExtra(TextToSpeech.Engine.EXTRA_AVAILABLE_VOICES);
                    if (available != null && available.size() > 0) {
                        updateDefaultLocalePref(currentEngine, available);
                    }
        		}
        	}
        	maybeUpdateTtsLanguage(currentEngine);
        	currentLocale = mTts.getLanguage();
        	Log.d(TAG, "get new currentLocale = " + currentLocale);
        }
//modify GPBYL-123 20130522_maoyutao end
...
//modify GPBYL-123 20130522_maoyutao start
    private Intent mVoiceDataDetails = null;
    private void updateDefaultLocalePref(String currentEngine, ArrayList<String> availableLangs) {
    	Log.d(TAG, "availableLangs.get(0) = " + availableLangs.get(0));
        mEnginesHelper.updateLocalePrefForEngine(currentEngine, availableLangs.get(0));
    }
//modify GPBYL-123 20130522_maoyutao end
