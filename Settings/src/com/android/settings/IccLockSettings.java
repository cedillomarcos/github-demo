...
public class IccLockSettings extends PreferenceActivity
    ...
    boolean isIccLockEnabled() {
        ...
        if (!FeatureOption.MTK_GEMINI_SUPPORT) {
            ...
            if (PhoneConstants.GEMINI_SIM_1 == mSimId || PhoneConstants.GEMINI_SIM_2 == mSimId) {
    ...
    private void tryChangeIccLockState() {
        ...
        if (!FeatureOption.MTK_GEMINI_SUPPORT) {
            ...
            if (PhoneConstants.GEMINI_SIM_1 == mSimId) {
                mGeminiPhone.getIccCardGemini(PhoneConstants.GEMINI_SIM_1).setIccLockEnabled(mToState, mPin, callback);
                Xlog.d(TAG, "tryChangeIccLockState [SIM 1]");
                // Disable the setting till the response is received.
                mPinToggle.setEnabled(false);
            } else if (PhoneConstants.GEMINI_SIM_2 == mSimId) {
                mGeminiPhone.getIccCardGemini(PhoneConstants.GEMINI_SIM_2).setIccLockEnabled(mToState,mPin,callback);
    ...
    */
    ...
    private void tryChangePin() {
        ...
        if (!FeatureOption.MTK_GEMINI_SUPPORT) {
            ...
            if (PhoneConstants.GEMINI_SIM_1 == mSimId) {
                mGeminiPhone.getIccCardGemini(PhoneConstants.GEMINI_SIM_1).changeIccLockPassword(mOldPin,mNewPin, callback);
                Xlog.d(TAG, "tryChangePin [SIM 1]");
            } else if (PhoneConstants.GEMINI_SIM_2 == mSimId) {
                mGeminiPhone.getIccCardGemini(PhoneConstants.GEMINI_SIM_2).changeIccLockPassword(mOldPin,mNewPin, callback);
