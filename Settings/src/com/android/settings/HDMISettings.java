...
public class HDMISettings extends SettingsPreferenceFragment implements
    ...
    private CharSequence[] defaultResolutionEntries;
    private CharSequence[] defaultResolutionValues;
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (FeatureOption.MTK_MT8193_HDMI_SUPPORT) {
                if (HDMILocalService.ACTION_EDID_UPDATED.equals(action)) {
                    Xlog.e(TAG, "HDMISettings, ACTION_EDID_UPDATED is received");
                    updatePref();
                }
            }
        }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (!FeatureOption.MTK_MT8193_HDMI_SUPPORT) {
            ...
            defaultResolutionEntries = mVideoResolutionPref.getEntries();
            defaultResolutionValues = mVideoResolutionPref.getEntryValues();            
        }
        ...
        if (FeatureOption.MTK_MT8193_HDMI_SUPPORT) {
            filter.addAction(HDMILocalService.ACTION_EDID_UPDATED);
        }
    ...
    private void updateSelectedResolution() {
        ...
        CharSequence[] resolutionEntries = mVideoResolutionPref.getEntries();
        ...
        if (FeatureOption.MTK_MT8193_HDMI_SUPPORT) {
            ...
            realResolution.add(defaultResolutionEntries[0]);
            realResolutionValues.add(defaultResolutionValues[0]);

            if (mHDMIService.sEdid != null) {
                //NTSC 720P 60Hz
                if (1 == ((mHDMIService.sEdid[0] >> 1) & 0x01)) {
                    Xlog.i(TAG, "NTSC 720P 60Hz");
                    realResolution.add(defaultResolutionEntries[1]);
                    realResolutionValues.add(defaultResolutionValues[1]);
                }

                //PAL 720P 50Hz
                if (1 == ((mHDMIService.sEdid[1] >> 11) & 0x01)) {
                    Xlog.i(TAG, "PAL 720P 50Hz");
                    realResolution.add(defaultResolutionEntries[2]);
                    realResolutionValues.add(defaultResolutionValues[2]);
                }
                
                //NTSC 1080P 23.976Hz or PAL 1080P 24Hz
                if ((1 == ((mHDMIService.sEdid[0] >> 21) & 0x01))
                        || (1 == ((mHDMIService.sEdid[1] >> 20) & 0x01))) {
                    Xlog.i(TAG, "NTSC 1080P 23.976Hz or PAL 1080P 24Hz");
                    realResolution.add(defaultResolutionEntries[3]);
                    realResolutionValues.add(defaultResolutionValues[3]);
                }
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        ...
        if (KEY_TOGGLE_HDMI.equals(key)) {
            ...
            if (newResolution.equals("100")) { //Auto
                if (mHDMIService.sEdid != null) {  
//   GPBYL-739(start)           MTK      Patch 
               if(!FeatureOption.RLK_HDMI_89M_PATH)
               {
                    ...
                    } else if (1 == (mHDMIService.sEdid[0] & 0x01)) 
		    {
               ...
               {  
		        //Priority: 480p > 576p
		    if (1 == (mHDMIService.sEdid[0] & 0x01))
		    {
                        //NTSC 480P (UI shows Auto)
                        newResolution = "100";
                    } else if (1 == ((mHDMIService.sEdid[1] >> 10) & 0x01)) {
