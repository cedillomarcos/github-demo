...
public class VideoFavoritesWidgetTest extends ActivityInstrumentationTestCase2<VFWidgetTestActivity> {
    ...
    public void test040AddIcon() {
        ...
        mSolo.clickOnView(addIcon);

        if (mSolo.waitForActivity("WidgetActionActivity", 5000) == false) {
            ArrayList<Activity> list = mSolo.getAllOpenedActivities();

            int i = 0;
            for (Activity a : list) {
                Xlog.v(TAG, "Activity opening# " + i + ": " +  a.getLocalClassName());
            }
            assertTrue(false);
        }
    }
