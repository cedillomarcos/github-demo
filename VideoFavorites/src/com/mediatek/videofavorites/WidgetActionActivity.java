...
public class WidgetActionActivity extends Activity implements
    ...
    private static int sRecorderQuality = 0;
    ...
    private int getRecorderQuality() {
        ...
        if (CamcorderProfile.hasProfile(CamcorderProfile.QUALITY_MTK_HIGH)) {
            profile = CamcorderProfile.getMtk(CamcorderProfile.QUALITY_MTK_HIGH);
        ...
        if (CamcorderProfile.hasProfile(CamcorderProfile.QUALITY_MTK_MEDIUM)) {
            profile = CamcorderProfile.getMtk(CamcorderProfile.QUALITY_MTK_MEDIUM);
            if (isVideoResolutionMatch(profile, VIDEO_WIDTH, VIDEO_HEIGHT)) {
    ...
    private class TranscodeTask extends AsyncTask<Uri, Integer, String> {
	    ...
	    //GPBYL-19 20130516 IllegalArgumentException to Exception (start)
	    //} catch (IllegalArgumentException ex) {
            } catch (Exception ex) {
	    //GPBYL-19 20130516 IllegalArgumentException to Exception (end)
        ...
        protected void onPostExecute(String outPathName) {
			...
			//GPBYL-344 liyang 20130531 (on)
                v.put(Video.Media.DURATION, "10000");
			//GPBYL-344 liyang 20130531 (off)
