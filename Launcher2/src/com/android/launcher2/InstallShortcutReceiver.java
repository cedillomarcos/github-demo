        ...
        if (findEmptyCell(context, items, tmpCoordinates, screen)) {
            if (intent != null) {
                ...
                if (duplicate || !shortcutExists) {
                    ...
                    try {
						String nameStr = data.getStringExtra(Intent.EXTRA_SHORTCUT_NAME);
						messageSuccess.append(String.format(context.getString(R.string.shortcut_installed), nameStr));
					} catch (Exception e) {
						messageSuccess.append("Shortcut created.");
						LauncherLog.d("zsc", "InstallShortcutReceiver.installShortcut()--get messageSuccess Exception!!!");
						e.printStackTrace();
					}
                    Toast.makeText(context, messageSuccess.toString(), Toast.LENGTH_SHORT).show();
