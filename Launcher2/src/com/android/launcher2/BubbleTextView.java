...
public class BubbleTextView extends TextView {
	...
	private static final int extraDeleteIconRegion = 15;
	private static final int MESSAGE_CANCEL_ANIM = 1;
	private static final int MESSAGE_ROTATE = 2;
    private float mScaleX = 1.0f;
    private float mScaleY = 1.0f;
    private static final Random mRandom = new Random();
    private int mTranslateX;
    private int mTranslateY;
    boolean isAllAppsButton;
...
//add zhaojiangwei test
	...
	//add zhaojiangwei test
	if (false){
	Intent mIntent = new Intent(info.intent);
	boolean isSpecialIntentAction = Intent.ACTION_CALL.equals(mIntent.getAction()) ||
		 Intent.ACTION_SENDTO.equals(mIntent.getAction()) ||
		 "com.android.contacts.action.QUICK_CONTACT".equals(mIntent.getAction());
	boolean inPowerMode = Settings.Global.getInt(mContext.getContentResolver(),
 Settings.Global.POWER_MODE_ON, 0) != 0;
	String packageName = info.getPackageName();
	if ("com.android.browser".equals(packageName) && inPowerMode){
		if(!isSpecialIntentAction)		
		b = Utilities.drawDisabledBitmap(b,mContext);
	}
	}
	//add zhaojiangwei test
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                //zhuwei add
                if (null != mLauncher && mLauncher.canShowUninstallMode() && getIsAnimating()) {
                	/*if (isDeleteShortcut) {
                		isDeleteShortcut = false;
                	}*/
                	...
                	left = getScrollX();
                	top = getScrollY();
                	right = left + iconDeleteBgWidth + extraDeleteIconRegion;
                	bottom = top + iconDeletegHeight + extraDeleteIconRegion;
    ...
    private final Handler mHandler = new Handler() {
  		...
  		public void handleMessage(Message msg) {
  			if (msg.what == MESSAGE_CANCEL_ANIM) {
  		        Log.e("zhuwei", "BubbleTextView.mHandler--invalidate()--item="+getText());
     ...
     public void drawDeleteIcon(Canvas canvas){
          ...
          if (null != mLauncher && mLauncher.canShowUninstallMode() && getIsAnimating()) {
          	if (mIconDelete != null && mIconDeleteBgBounds != null) {
          		int iconDeleteBgPosX = getScrollX() + mTranslateX;
          		int iconDeleteBgPosY = getScrollY() + mTranslateY;
      ...
      public void startRotateAnimation() {
      	if (isAllAppsButton) {
      		Log.e("zhuwei", "BubbleTextView startRotateAnimation isAllAppsButton--> "+isAllAppsButton);
      ...
      private void initAnim() {
      	...
      	AnimatorUpdateListener update = new AnimatorUpdateListener() {
      		...
      		public void onAnimationUpdate(ValueAnimator animation) {
      			...
      			if (null != mLauncher && mRotateAnim.isRunning() && !mLauncher.canShowUninstallMode()) {
