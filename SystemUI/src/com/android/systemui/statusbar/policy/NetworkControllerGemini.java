...
public class NetworkControllerGemini extends BroadcastReceiver {
    ...
    /*GPBYB-267 shuchenlin begin*/
    private static int updateTelephonySignalStrengthCount=0;
    /*GPBYB-267 shuchenlin end*/
    // yuck -- stop doing this here and put it in the framework
    private IBatteryStats mBatteryStats;
//renxinquan add GPBYL-937 start
	private static int[] mLevel1 = {0, 0, 0, 0};	
	private static int[] mLevel2 = {0, 0, 0, 0};	
	private static int old_mLevel1 = 0;	
	private static int old_mLevel2 = 0;
//renxinquan add GPBYL-937 end
    ...
    private final void updateTelephonySignalStrength(int slotId) {
        ...
        if (!handled && !isSimInserted(slotId)) {
            ...
            if (resId != -1) {
                ...
                /*GPBYB-267 shuchenlin begin*/
                //the number of updateTelephonySignalStrength is maybe not 5 in other products when boot compeleted.
                if (FeatureOption.RLK_GP818H_A1_SN_SUPPORT) {
                    if(updateTelephonySignalStrengthCount >= 5) {
                        tempPhoneSignalIconId[0].setIconId(R.drawable.stat_sys_gemini_signal_null);
                    } else {
                        updateTelephonySignalStrengthCount++;
        ...
        if (!handled) {
            ...
            if (!hasService) {
                ...
                if (tempSignalStrength == null) {
                    ...
                    //renxinquan add GPBYL-937 start
						if (com.mediatek.common.featureoption.FeatureOption.RLK_INDONESIA_NETWORK_SUPPORT) {
						int level_total = 0;
						int i = 0;
						boolean array_ok = false;
						int level = tempSignalStrength.getLevel();
						Xlog.d("GSLWSW-334", "real level:" + level);

						if (slotId == 0) {
							for (i = 0; i < mLevel1.length; i++) {
								if (mLevel1[i] == 0) {
									mLevel1[i] = level;
									array_ok = true;
									break;
							...
							Xlog.d("GSLWSW-334", "getLevelExt, array_ok=" + array_ok);
							if (!array_ok) {
								for (i = 0; i < mLevel1.length - 1; i++) {
									mLevel1[i] = mLevel1[i + 1];
								}

								mLevel1[mLevel1.length - 1] = level;

								for (i = 0; i < mLevel1.length; i++) {
									Xlog.d("GSLWSW-334", "getLevelExt, mLevel1[" + i + "]=" + mLevel1[i]);
									level_total += mLevel1[i];
								}
								Xlog.d("GSLWSW-334", "getLevelExt, level_total=" + level_total);

								if (level > old_mLevel1) {
									old_mLevel1 = level;
								} else {
									level = (level_total + mLevel1.length / 2) / mLevel1.length;
									old_mLevel1 = level;
								}
							}
						} else if (slotId == 1) {
							for (i = 0; i < mLevel2.length; i++) {
								if (mLevel2[i] == 0) {
									mLevel2[i] = level;
									array_ok = true;
									break;
							...
							Xlog.d("GSLWSW-334", "getLevelExt, array_ok=" + array_ok);
							if (!array_ok) {
								for (i = 0; i < mLevel2.length - 1; i++) {
									mLevel2[i] = mLevel2[i + 1];
								}

								mLevel2[mLevel2.length - 1] = level;

								for (i = 0; i < mLevel2.length; i++) {
									Xlog.d("GSLWSW-334", "getLevelExt, mLevel2[" + i + "]=" + mLevel2[i]);
									level_total += mLevel2[i];
								}
								Xlog.d("GSLWSW-334", "getLevelExt, level_total=" + level_total);

								if (level > old_mLevel2) {
									old_mLevel2 = level;
								} else {
									level = (level_total + mLevel2.length / 2) / mLevel2.length;
									old_mLevel2 = level;
								}
							}
						}

						Xlog.d("GSLWSW-334", "show level:" + level);
						tempLastSignalLevel[0] = iconLevel[0] = level;
						} else {
