...
//    };
    ...
    private final Runnable mProgressChecker = new Runnable() {
        ...
        public void run() {
            int pos = setProgress();
	    //GPBYL-62 liyang 20130521 (on)
            //mHandler.postDelayed(mProgressChecker, 1000 - (pos % 1000));
            mHandler.postDelayed(mProgressChecker, 100 - (pos % 100));
	    //GPBYL-62 liyang 20130521 (off)
        }
    };
    ...
    public boolean onInfo(MediaPlayer mp, int what, int extra) {
        ...
        /// M: add deal with MEDIA_INFO_AUDIO_NOT_SUPPORTED status @{
        if (mFirstBePlayed && what == MediaPlayer.MEDIA_INFO_AUDIO_NOT_SUPPORTED) {
            Toast.makeText(mActivityContext, R.string.audio_not_supported, Toast.LENGTH_SHORT).show();
            mFirstBePlayed = false;
            return true;
        }
        /// @}
    ...
    private class ScreenModeExt implements Restorable, ScreenModeListener {
		...
		//GPBYB-302 liyang 20130909 change start
        //private int mScreenMode = ScreenModeManager.SCREENMODE_CROPSCREEN;
        private int mScreenMode;// = ScreenModeManager.SCREENMODE_BIGSCREEN;
       
		{
			if (MovieActivity.isLandScape){
				mScreenMode = ScreenModeManager.SCREENMODE_CROPSCREEN;
			}else{
				mScreenMode = ScreenModeManager.SCREENMODE_BIGSCREEN;
			}
		
		}
		//GPBYB-302 liyang 20130909 change end
