...
public class PositionController {
    ...
    public void setViewSize(int viewW, int viewH) {
        ...
        if (DEBUG_PROFILE_PERFORMANCE) {
            MediatekMMProfile.triggerProfileSetView(viewW + "x" + viewH);
        }
        ...
        if (wasMinimal) {
            ...
            if (DEBUG_PROFILE_PERFORMANCE) {
                MediatekMMProfile.triggerProfileSetView( 
                        "mCurrentScale=" + b.mCurrentScale + "mScaleMin=" + b.mScaleMin);
            }
        }
    ...
    public void setImageSize(int index, Size s, Rect cFrame) {
        if (s.width == 0 || s.height == 0) return;
        if (DEBUG_PROFILE_PERFORMANCE) {
            MediatekMMProfile.triggerProfileSetImageSize("index=" + index +
                    ",size=[" + s.width + "x" + s.height + "], cFrame=" + cFrame);
        }
    ...
    private boolean setBoxSize(int i, int width, int height, boolean isViewSize) {
        if (DEBUG_PROFILE_PERFORMANCE) {
            MediatekMMProfile.triggerProfileBoxInfo("setBox[" + i + "].Size:"
                    + width + "x" + height + "//isViewSize=" + isViewSize);
        }
        Box b = mBoxes.get(i);
    ...
    private boolean startAnimation(int targetX, int targetY, float targetScale,
            int kind) {
        if (DEBUG_PROFILE_PERFORMANCE) {
            MediatekMMProfile.triggerProfileStartAnimation("targetX=" + targetX +
                    ",targetY=" + targetY + ",targetScale=" + targetScale +
                    ",kind=" + getAnimLabel(kind));
        }
    ...
    private class Box extends Animatable {
        ...
        private boolean doAnimation(int targetY, float targetScale, int kind) {
            ...
            if (DEBUG_PROFILE_PERFORMANCE) {
                MediatekMMProfile.triggerProfileBoxAnim("targetY=" + targetY +
                        ",targetScale=" + targetScale + ",kind=" + getAnimLabel(kind));
            }
    ...
    private static String getAnimLabel(int kind) {
        switch (kind) {
            case ANIM_KIND_NONE: return "ANIM_KIND_NONE";
            case ANIM_KIND_SCROLL: return "ANIM_KIND_SCROLL";
            case ANIM_KIND_SCALE: return "ANIM_KIND_SCALE";
            case ANIM_KIND_SNAPBACK: return "ANIM_KIND_SNAPBACK";
            case ANIM_KIND_SLIDE: return "ANIM_KIND_SLIDE";
            case ANIM_KIND_ZOOM: return "ANIM_KIND_ZOOM";
            case ANIM_KIND_OPENING: return "ANIM_KIND_OPENING";
            case ANIM_KIND_FLING: return "ANIM_KIND_FLING";
            case ANIM_KIND_FLING_X: return "ANIM_KIND_FLING_X";
            case ANIM_KIND_DELETE: return "ANIM_KIND_DELETE";
            case ANIM_KIND_CAPTURE: return "ANIM_KIND_CAPTURE";
        }
