...
public class SelectionManager {
    ...
    public ArrayList<Path> getSelected(boolean expandSet) {
        ...
        if (mIsAlbumSet) {
            ...
            /// M: when mSourceMediaSet != null, we check if items in click set are still in
            // mSourceMediaSet, if not, we do not add it to selected list @{
            if(mSourceMediaSet != null) {
                ...
                while (index < total) {
                    ...
                    for (MediaItem item : list) {
                        ...
                        if ((mInverseSelection && !mClickedSet.contains(id))
                                || (!mInverseSelection && mClickedSet.contains(id))) {
                            selected.add(id);
                        }
                    }
                    index += count;
