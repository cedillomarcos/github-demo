...
// add BUG_ID:GBLLSW-551 20130322 huangyujie (start)
    public static final String KEY_STORAGE_LOCATION = "pref_camera_storage_location";
// add BUG_ID:GBLLSW-551 20130322 huangyujie (end)
    ...
    // add BUG_ID:GBLLSW-551 20130322 huangyujie (start)
        ListPreference storages = group.findPreference(KEY_STORAGE_LOCATION);
    // add BUG_ID:GBLLSW-551 20130322 huangyujie (end)
        ...
        // add BUG_ID:GBLLSW-551 20130322 huangyujie (start)
            if(Storage.isHaveExternalSDCard()) {
                settingChecker.setListPreference(SettingChecker.ROW_SETTING_STORAGE, storages);
            }
        // add BUG_ID:GBLLSW-551 20130322 huangyujie (end)
            ...
            //remove facebeauty properties if smooth not exists.
            if (facebeautyproperties != null && !ParametersHelper.isSupportedSmooth(mParameters)) {
            ...
            if (videoEffect != null) {
                if (!FeatureSwitcher.isVideoLiveEffectEnabled() || (!FeatureSwitcher.isFrontVideoLiveEffectEnabled()
                        && SettingChecker.CAMERA_FRONT_ID == mContext.getCameraId())) {
    ...
    public static double findFullscreenRatio(Context context, Parameters parameters) {
        ...
        if (context != null && parameters != null) {
            ...
            List<Size> supportedAllList = parameters.getSupportedPictureSizes();
            if (supportedAllList != null) {
                for (Size size : supportedAllList) {
                    double ratio = (double)size.width / size.height;
                    if (Math.abs(fullscreen - ratio) < Math.abs(fullscreen - find)) {
                        find = ratio;
                    }
                    if (LOG) {
                        Log.v(TAG, "findFullscreenRatio() fullscreen=" + fullscreen + ", ratio=" + ratio
                                + ", find=" + find + ", size=" + SettingUtils.buildSize(size));
                    }
                }
            }
        }
    ...
    private void filterUnsupportedOptions(PreferenceGroup group,
        ...
        // only one options for the settings.
            Log.v("liyang", "supported: " + supported + ", supported.size(): " + supported.size());
        ...
        pref.filterUnsupported(supported);
            Log.v("liyang", "pref.getEntries().length: " + pref.getEntries().length);
