...
public class Camera extends ActivityBase implements PreviewFrameLayout.OnSizeChangedListener,
    ...
    // this variable is workaround to let notify parameter take place when the
    // next message is handled.
    private boolean mNeedNotifyParameterReady;
...
//                break;
            ...
            // M: we call notify function each time in handleMessage() in order to
            // to process MSG_CAMERA_PARAMETERS_READY message earlier to let Camera
            // launch faster.
            notifyParametersReady();
            if (LOG) {
    ...
    private void applyParameters(boolean force) {
        ...
        if (needRestart) {
            ...
            if (!isSwitchingCamera() && changedPreviewSize && mSurfaceTexture != null) {
        ...
        mNeedNotifyParameterReady = true;
    ...
    protected void onAfterFullScreeChanged(final boolean full) {
        ...
        if (full) {
            /* M: MSG_CHECK_DISPLAY_ROTATION may get a rong display rotation in this case:
    ...
    // modify BUG_ID:GBLLSW-551 20130322 huangyujie (start)
        ...
        Storage.updateDefaultDirectory(Camera.this, mCameraId);
    ...
    public void onBackPressed() {
        ...
        if (!mCameraActor.onBackPressed() && !collapseViewManager(false)) {
            super.onBackPressed();
			//GPBYL-179 liyang 20130517 (on)
            //clearUserSettings();
			//GPBYL-179 liyang 20130517 (off)
        }
...
//    }
    ...
    private void notifyParametersReady() {
        if (!mNeedNotifyParameterReady) {
            return;
        }
        mNeedNotifyParameterReady = false;
    ...
    private SettingManager.SettingListener mSettingListener = new SettingManager.SettingListener() {
        ...
        // add BUG_ID:GBLLSW-551 20130322 huangyujie (start)
            if (isMountPointChanged() && Storage.isHaveExternalSDCard()) {

            	ComboPreferences.Editor editor = mPreferences.edit();
                editor.putString(CameraSettings.KEY_STORAGE_LOCATION, Storage.getMountPoint());
                editor.commit();
                editor.apply();
                ...
                Runnable runnableOK = new Runnable() {
                    ...
                    public void run() {
                        ...
                        i.setClass(Camera.this, Camera.class);
                        startActivity(i);
                        finish();
                    }
                };
                ...
                mRotateDialog.showAlertDialog(
                        getString(R.string.storage_location_resstart_title),
                        getString(R.string.storage_location_resstart_massage),
                        getString(android.R.string.ok), runnableOK,
                        getString(android.R.string.cancel), runnableCancel);
                return;
        ...
        public void onRestorePreferencesClicked() {
            ...
            Runnable runnable = new Runnable() {
                ...
                // add BUG_ID:GBLLSW-551 20130322 huangyujie (start)
                    Storage.updateDefaultDirectory(Camera.this, 0);
                    Storage.updateDefaultDirectory(Camera.this, 1);
		    //GPBYB-76 liyang 20130809 start
                    /*Intent i = new Intent();
                    i.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
                    i.setClass(Camera.this, Camera.class);
                    startActivity(i);
                    finish();*/
		    //GPBYB-76 liyang 20130809 end	
                // add BUG_ID:GBLLSW-551 20130322 huangyujie (end)
                }
            };
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		//modify GBLSSW-703 hjx 20130401
        if (isFullScreen() && KeyEvent.KEYCODE_MENU == keyCode && event.getRepeatCount() == 0
                && (mRotateDialog!=null&&!mRotateDialog.isShowing())&& mSettingManager.handleMenuEvent()) {
            return true;
        }
    ...
    public void setResultExAndFinish(int resultCode) {
        ...
        finish();
	//GPBYB-54 liyang 20130809 start
        //clearUserSettings();
	//GPBYB-54 liyang 20130809 end
    }
        ...
        runOnUiThread(new Runnable() {
           @Override
            public void run() {
               ...
               if (!isVideoMode() && isVoiceEnabled() && isAppGuideFinished()) {
                   showToast(userGuide);
               }
            } 
        });
